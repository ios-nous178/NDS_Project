...
> **Codex skill (생성됨).** SSOT 는 `.claude/skills/ds-audit/SKILL.md` — 직접 수정 금지, SSOT 수정 후 `pnpm sync:skills`. 명시 호출 `/skills` → `ds-audit` 또는 `$ds-audit [Component|all]`. 기본 읽기 전용(임의 수정/커밋 금지).
