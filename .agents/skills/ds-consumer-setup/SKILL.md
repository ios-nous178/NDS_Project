...
description: >-
  ...
  AGENTS.md/CLAUDE.md 가이드 주입, 프로젝트 SSOT 마커, 패키지 설치(어드민만), import 배선, DsInspector 주입까지
...
> MCP 의 `get_setup` 도구가 수행한다 — 이 스킬은 스텝 순서·intent/project 판별·검증을 책임진다.
...
- **project** — `trost | geniet | nudge-eap | cashwalk-biz | runmile`. 주면 `get_setup` 이 cwd 에 프로젝트 SSOT 마커(`nudge.project`)를 박아 빌드가 단일 출처에서 읽는다. HTML 은 `<html data-project="<slug>">` 로도 지정.

먼저 사용자/레포 단서로 intent·project 를 추정하고, 모호하면 한 번 물어본 뒤 진행한다.
...
| `claude-md` | cwd 에 `CLAUDE.md` 생성(가이드 SSOT). `intent`/`project`/`template:'slim'\|'default'`      | Claude Code 로 작업할 워크스페이스                      |
...
- intent·project 추정/확인.

### Phase 1 — 전체 그림

- `get_setup({ step:'full', intent, project })`(요약)으로 절차 개요를 받고 사용자에게 무엇이 일어날지 먼저 알린다. 필요하면 `mode:'full'`.

### Phase 2 — 가이드 주입 (가장 중요)

- **Codex 워크스페이스: `get_setup({ step:'agents-md', cwd, projectName, intent, project })`** → `AGENTS.md` 생성. (Claude Code 면 `claude-md`.) 둘 다 필요하면 둘 다.
- 기존 파일 있으면 preview 먼저 보여주고 overwrite 여부를 사용자에게 확인.
- 생성 후 안내: **에이전트 세션을 재시작/리로드**해야 새 가이드가 반영됨(도구가 `next` 로 알려줌).

### Phase 3 — 트랙별 배선

- **html intent** — 설치/ import 불필요. `index.html` 에 `<html data-project="<slug>">` + `<nds-*>` 작성 → `build_singlefile_html` 로 DS runtime/CSS inline 됨을 안내. 비율 점검은 `validate_html_mockup({ filePath, withStats:true })`.
- **admin-cms intent** —
  1. `get_setup({ step:'install', intent:'admin-cms', includeTailwind? })` → npm 설치 커맨드 실행.
  2. `get_setup({ step:'imports', project })` → `src/main.tsx` 토큰/스타일/runtime import 배선.
...
- 생성된 `AGENTS.md`/`CLAUDE.md` 존재·프로젝트 마커(`nudge.project`) 확인.
...
- 프로젝트 마커 누락 → 빌드가 프로젝트를 못 잡음. project 인자를 빼먹지 말 것.
