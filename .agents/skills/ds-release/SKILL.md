...
> **Codex skill (생성됨).** SSOT 는 `.claude/skills/ds-release/SKILL.md` — 직접 수정 금지, SSOT 수정 후 `pnpm sync:skills`. 명시 호출 `/skills` → `ds-release` 또는 `$ds-release`. main push 는 사용자 확인 후에만.
