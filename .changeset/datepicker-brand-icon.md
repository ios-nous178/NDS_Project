---
"@nudge-eap/styles": patch
"@nudge-eap/html": patch
---

`<nds-date-picker>` (그리고 내부적으로 이 컴포넌트를 쓰는 `<nds-date-range-picker>`) 의 캘린더 트리거 아이콘을 `[data-brand]` cascade 로 brand 별 swap. React 의 `useBrand()` 분기와 1:1 정합.
