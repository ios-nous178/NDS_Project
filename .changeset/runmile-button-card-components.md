...
- **버튼** (ButtonGuide 5124:390): Solid/Neutral = 검정(#221E1F) + 흰 텍스트(기존 gray200 오매핑 수정), 5사이즈 높이(Mini40/S44/M48/L52/XL56). Secondary tone 없음 — 검정/회색 CTA 는 `color="neutral"`(solid=검정 / soft=회색). `color="secondary"` 사용 시 dev 경고 + validator `project-denied-button-color`(project-profiles 에 runmile cta 정책 선언).
