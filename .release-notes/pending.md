...
· *트로스트(Trost)* 와 *지니어트(Geniet)* 두 프로젝트를 Figma 가이드 기준으로 본격 반영 — 컴포넌트·색·아이콘·로고·이미지 자산까지.
· 잘 안 쓰이던 컴포넌트·중복 코드를 정리해 라이브러리를 더 가볍고 일관되게.
· 프로젝트 이미지 검색에 *한글* 지원 추가.

*주요 변경사항*

*1. 트로스트(Trost) 프로젝트 본격 합류*
...
· 기존 사용법과 다른 프로젝트 화면은 그대로 — 트로스트 값은 새 옵션이나 프로젝트 토큰으로만 더했습니다.

*2. 지니어트(Geniet) 프로젝트 정합 확장*
...
· 프로젝트 자산 *146종* 편입 — 음식 종류·카테고리·프로필·워드마크.
...
*5. 프로젝트 헤더·푸터·사이드바 구조 정리*
· 프로젝트 chrome(헤더·푸터·사이드바)을 목업 전용 구조로 통합 정리.
· 모바일 하단 탭 바 *BottomNav* 를 프로젝트 무관 공식 컴포넌트로 추가.

*6. 프로젝트 차이를 토큰으로 이전*
· Snackbar·Toggle·Pagination·TagInput 등의 프로젝트별 색·radius 분기를 토큰 슬롯으로 이전.
· → 새 프로젝트를 추가할 때 컴포넌트를 안 건드리고 토큰 값만 더하면 되도록 구조화.

*7. 프로젝트 이미지 한글 검색(find_asset)*
· "비빔밥·족발·김치·라면·돈까스·우울증·영양제·한식" 처럼 *한글 키워드로 프로젝트 이미지 검색* 지원(기존엔 영문 이름만).
...
· 트로스트·지니어트 화면을 DS 컴포넌트만으로 프로젝트 정합하게 제작할 수 있습니다.
