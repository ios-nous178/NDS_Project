...
너는 **멀티프로젝트 디자인시스템(DS)을 실제로 운영·확장해본 시니어 DS 아키텍트**다.
...
EAP(직원 멘탈케어) 플랫폼용 디자인시스템 모노레포. **5개 프로젝트**를 한 코드베이스로 지원: Trost / Geniet / NudgeEAP / CashwalkBiz / Runmile.
...
- Storybook 스토리: **147개**
- 프로젝트 토큰 세트: 5개 (palette + semantic 분리)
...
- 즉 문서가 아니라 **실행 가능한 MCP 도구**가 진리의 원천: `get_guide`, `find_component`, `find_icon`, `find_token`, `get_project`, `build_singlefile_html`, `convert_html_to_ds_html`, `list_figma_sync_status` 등.
...
- `--nds-*` — DS 자체 컴포넌트 슬롯 (sidebar, chip, tooltip-bg, 프로젝트별 슬롯 등).
- raw hex 신규 추가 금지. 프로젝트별 색은 data-project cascade가 아니라 **별도 :root CSS 파일 선택**으로 적용.

**4) 프로젝트 차별화 방식**
- 토큰: 프로젝트마다 `{project}.palette.ts` + `{project}.semantic.ts`.
- 같은 컴포넌트가 프로젝트마다 다른 의미를 가질 수 있음. 예: "검정 secondary CTA"가 Geniet은 `color=secondary`, Cashpobi는 `color=neutral` (denylist + validator 경고로 가드).
- 프로젝트 아이콘 글리프는 `[data-project]` cascade + CSS mask로 swap.
...
3. **5개 프로젝트를 한 컴포넌트 셋으로 흡수하면서 "프로젝트별 의미 차이"(neutral vs secondary 등)를 denylist+validator 경고로 가드하는 방식.**
   6번째 프로젝트가 들어오면 이 모델이 깨지나? 프로젝트별 예외가 누적되면 어디서 터지나?
...
- 프로젝트 토큰 세트 1개 (`cashwalk-biz.semantic.ts`)
- 루트 `CLAUDE.md` 전문
