...
CI 실패의 최다 원인은 **생성물 커밋 누락** — `--check` 게이트(개수는 `scripts/gates.mjs` 가 SSOT, 현재 26개)는 검증만 하고 재생성은 안 해준다.
...
5. **검증은 마지막에 한 번** — 다단계 작업(컴포넌트 제거·머지 등) 중 매 편집마다 `pnpm typecheck`/`pnpm fix`(둘 다 풀 빌드, 50~75초)를 돌리지 말 것. 소스 편집을 **전부 끝낸 뒤 커밋 직전에 한 번만** 돌린다. 중간 확인은 변경 파일만 `tsc`/대상 게이트로 빠르게. 생성물 재생성도 `pnpm fix` 한 번이 SSOT 라 반복은 시간 낭비.
