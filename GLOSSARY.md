...
같은 정보를 **한 곳에서만** 정의하고 나머지는 거기서 파생하는 원칙. 예: 컴포넌트 Props 의 SSOT 는 `packages/react/src`, DS 사용 규칙의 SSOT 는 MCP, 브랜드 색의 SSOT 는 `tokens/src/brands/`. "이거 어디서 고쳐?" 의 답이 항상 한 곳이어야 합니다.
...
### `--nds-*` 토큰 (컴포넌트 슬롯)
DS 자체 컴포넌트의 전용 슬롯 토큰. 예: `--nds-snackbar-bg`, `--nds-chip-*`. 브랜드 override 와 variant 색을 **우선순위 var() 체인**으로 합성하는 자리입니다. → CLAUDE.md "색은 슬롯에 넣고 우선순위로 합성"

### brand cascade (브랜드 캐스케이드)
컴포넌트는 브랜드를 모르고 시멘틱/슬롯 토큰 하나만 참조 → **브랜드 토큰 파일이 그 값만 덮어써서** 컴포넌트로 흘려보내는 방식. `[data-brand="..."]` 색 분기를 컴포넌트에 박지 않는 게 핵심. silent base-fallback 은 `pnpm lint:brand-completeness` 가 차단.

### admission (DS 편입 기준)
컴포넌트를 DS 에 받아들일지 판단하는 기준: ① 2+ 브랜드 사용 or 명시 사유, ② **Figma 가이드 노드(`figmaNodeUrl`) 존재**, ③ 앱 비즈니스 로직 없음, ④ react+html 미러 동시 제공. → CLAUDE.md "DS 편입 기준".
...
### baseline (waiver)
"의도된 예외" 를 사유와 함께 박제하는 JSON. 예: `mirror-parity-baseline.json`(미러 divergence), `brand-completeness-baseline.json`(브랜드 미정의), `guide-figma-baseline.json`(figma 노드 미보유). 게이트는 **baseline 밖 신규** 만 차단합니다.
...
### catalog.json
react dist + html src 에서 생성되는 컴포넌트/아이콘/토큰/브랜드 **카탈로그**. MCP 도구·mirror-parity·component-attrs 의 입력.
