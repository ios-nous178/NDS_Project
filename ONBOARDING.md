...
- **무엇**: 5개 프로젝트(Trost · Geniet · NudgeEAP · CashwalkBiz · Runmile)가 공유하는 **디자인 시스템(DS)** 모노레포 — 토큰·React 컴포넌트·아이콘·문서.
...
- **용어가 낯설면**: [GLOSSARY.md](GLOSSARY.md) (SSOT · 3면 미러 · semantic 토큰 · project cascade 등).
...
4. 프로젝트 색은 `packages/tokens/src/projects/<project>.ts` 에서만 (컴포넌트 X) — [ARCHITECTURE.md](ARCHITECTURE.md) "프로젝트는 토큰으로만"
...
**Q. 왜 5개 프로젝트인가?**
EAP 멘탈케어 플랫폼이 프로젝트별로 운영되기 때문입니다. 프로젝트 차이는 **토큰 값 override** 로만 표현하고 컴포넌트는 프로젝트를 모릅니다. ([ARCHITECTURE.md](ARCHITECTURE.md))
...
- [GOVERNANCE.md](GOVERNANCE.md) — DS 운영·확장·개선 규칙 (무엇을 받고 / 어떻게 더 잘 만드나)
