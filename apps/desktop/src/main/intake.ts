...
import { canonicalPagePattern } from "@nudge-design/mockup-core/tools/standalone-assets";
...
export interface RunIntakeArgs {
  ...
  /**
   * 캐포비 어드민 Page Pattern(onboarding/dashboard/list/detail/form) — intake 추천 카드에서
   * 사용자가 고른 값. 유효한 5종이면 nudge.pagePattern 마커로 박아 save_design_spec 가 자동 주입한다.
   * (캐포비 어드민이 아니면 호출부가 보내지 않음.)
   */
  selectedPagePattern?: string;
}
...
function briefDoc(args: RunIntakeArgs, intent: string, docNames: string[]): string {
  // HTML 목업은 '읽을 문서'가 아니라 '재현할 정답 디자인'이므로 일반 첨부와 분리해 다른 지시를 붙인다.
  const htmlDocs = docNames.filter((n) => /\.html?$/i.test(n));
  const otherDocs = docNames.filter((n) => !/\.html?$/i.test(n));
  const htmlMockupSection = htmlDocs.length
    ? `\n## 기존 HTML 목업 (구조·시각 원본 — 재설계 아닌 재현)\n${htmlDocs
        .map((n) => `- brief/${n}`)
        .join(
          "\n",
  ...
  const pagePatternSection = pagePattern
    ? `\n## 확정 Page Pattern\n- **${pagePattern}** (사용자가 추천 카드에서 선택). save_design_spec 의 screen.pagePattern 에 이 값을 그대로 사용 — 재분류하지 말 것. 구조: get_guide({ topic: 'pattern:cashwalk-biz-page-${pagePattern}' }).\n`
    : "";
...
${directionSection}
${htmlMockupSection}${attachSection}`;
}
...
export function runIntake(args: RunIntakeArgs): RunIntakeResult {
  try {
    ...
    const hasVisual = screenshots.length > 0 || figmaUrls.length > 0 || hasHtmlMockup;

    // ── references.md (시각/HTML 원본 ≥1 일 때만 — 빈 references 로 게이트 거짓 충족 방지). ──
    if (hasVisual) {
      ...
      for (const name of savedHtmlDocs) {
        lines.push(
          `[good] source=brief/${name} caption=기존 HTML 목업(구조·콘텐츠·문구의 원본 — 충실히 재현)`,
        );
      }
    ...
    // save_design_spec 가 screen.pagePattern 미선언 시 이 마커에서 자동 주입(readPagePatternMarker).
    // 유효한 5종일 때만 박는다(오타/빈값 무시) — 모델이 재분류하거나 빠뜨릴 여지를 없앤다.
    const pagePattern = canonicalPagePattern(args.selectedPagePattern);
    if (pagePattern) {
      writeFileSync(join(workspaceDir, "nudge.pagePattern"), `${pagePattern}\n`, "utf8");
    }
    ...
    return {
      ...
      slug,
      intent,
      seedPrompt: seedPrompt(args, intent, hasVisual, hasHtmlMockup),
    };
  } catch (err) {
