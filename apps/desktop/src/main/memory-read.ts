 ...
 * 결정 로그를 읽어, 현재 브랜드의 최근 결정을 `--append-system-prompt` 에 얹는다 → 에이전트가
 * "지난번에 이 브랜드에서 이렇게 정했다"를 알고 일관성 있게 만든다.
 *
 * 설계 주의:
 *  - **브랜드로만 필터**한다. row.screen.surface 는 DesignSpec 어휘("web"|"app")이고 하네스의
 *    Surface 는 인테이크 어휘("service"|"admin")라 서로 매칭되지 않는다 — surface 필터는 모든 행을
 *    걸러내는 잠재 버그라 쓰지 않는다(브랜드 alias 는 canonicalBrandSlug 로 정규화해 비교).
...
export interface MemoryReadOptions {
  /** 현재 화면의 브랜드 — 같은 브랜드 결정만 주입한다(없으면 전체 풀에서 최근 결정). */
  brand?: string;
  /** 주입할 최대 행 수(기본 6). */
  maxRows?: number;
 ...
 * - 브랜드를 알면 같은 브랜드 결정만(다른 브랜드 결정은 주입하지 않아 오염을 막는다).
...
export function formatMemoryRead(
  ...
  // 브랜드 필터(alias 정규화). 브랜드를 아는데 같은 브랜드 결정이 없으면 주입하지 않는다.
  const target = canonicalBrandSlug(options.brand);
  if (target) {
    const sameBrand = pool.filter((r) => canonicalBrandSlug(r.screen?.brand) === target);
  ...
  const header = [
    ...
    "이전 세션에서 같은 브랜드 화면에 대해 내린 결정입니다. 일관성을 위해 특별한 이유가 없으면 이 결정을 따르고, 벗어나야 하면 그 이유를 save_design_spec 의 decisions/rationale 로 남기세요.",
  ].join("\n");
