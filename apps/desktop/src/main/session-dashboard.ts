...
import { appEventsLogPath, feedbackLogPath, readAppEvents, readFeedbackLog } from "@nudge-design/mockup-core";
...
function collectWorkspaceFiles(session: ChatSession, projectPath: string): SessionDashboardFile[] {
  const dir = workspaceDirs(session, projectPath).find((candidate) => existsSync(join(candidate, "brief.md")));
  if (!dir) return [];
...
function isDesignScoreMessage(msg: ChatMessage): msg is Extract<ChatMessage, { kind: "design-score" }> {
...
function collectLogFiles(projectPath: string): {
  ...
  return {
    sessionLogs: uniqueStrings(dirs.map((dir) => join(dir, CHAT_SESSIONS_FILENAME)).filter(existsSync)),
...
export function readSessionDashboard(projectPath: string): SessionDashboardResult {
  ...
  const feedbackEntries = dedupeFeedback(logFiles.feedbackLogs.flatMap((path) => readFeedbackLog(path)));
  ...
  const items: SessionDashboardItem[] = sessions.map((rawSession) => {
    ...
    const transcriptCandidates = dirs.map((dir) => join(dir, TRANSCRIPT_DIRNAME, `${session.sessionId}.log`));
