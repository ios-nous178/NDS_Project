  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        />
        <span
          style={{ fontSize: 11.5, fontWeight: 700, color: scoreColor(score.codeOverall ?? 0) }}
        >
          {codeText}
        </span>
...
export function SessionDashboardPanel({
  ...
  useEffect(() => {
    ...
    if (
      selectedSessionId &&
      dashboard.sessions.some((item) => item.session.sessionId === selectedSessionId)
    ) {
      return;
    }
  ...
  if (!dashboard)
    return (
      <div style={{ padding: 16, color: c.textFaint, fontSize: 13 }}>세션 대시보드 로딩 중…</div>
    );
              ...
              >
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
                  <div style={{ fontSize: 15, fontWeight: 700, color: c.text }}>
                    {selected.session.customTitle ??
                      selected.session.screenName ??
                  ...
                  <Chip
                    label={selected.session.status}
                    color={
                      selected.session.status === "failed"
                        ? c.red
                        : selected.session.status === "interrupted"
                          ? c.yellow
                          : c.green
                    }
                  />
                ...
                >
                  ...
                  <div>
                    mockup:{" "}
                    <span style={{ fontFamily: mono, color: c.text }}>
                      {selected.session.mockupFile ?? "-"}
                    </span>
                  </div>
                  <div>
                    native store:{" "}
                    <span style={{ fontFamily: mono, color: c.text }}>
                      {selected.session.agentSessionFile ?? "-"}
                    </span>
                  </div>
                </div>
              </section>
                    ...
                    {selected.feedback.map((entry) => (
                      ...
                      >
                        ...
                        >
                          ...
                          <span style={{ color: c.textMuted, fontSize: 11.5, fontFamily: mono }}>
                            {entry.reviewer ?? "unknown"}
                          </span>
                        </div>
                        <div
                          style={{
