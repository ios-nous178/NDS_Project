...
export function ValidationPanel({
  ...
  if (loading) return <div style={{ color: "#666", fontSize: 13 }}>검증 중…</div>;
  if (!result)
    return (
      <div style={{ color: "#999", fontSize: 13 }}>파일을 선택하면 검증 결과가 표시됩니다.</div>
    );
  ...
  return (
    <div style={{ fontSize: 13 }}>
      <div style={{ marginBottom: 12 }}>
        ...
        {result.ok && (
          <span style={{ color: "#067647", fontWeight: 600, marginLeft: 6 }}>✓ 위반 없음</span>
        )}
      </div>
      {violationsByRule.length === 0 ? (
        <div style={{ color: "#067647" }}>모든 규칙 통과.</div>
      ) : (
        <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
          {[...violationsByRule]
            .sort((a, b) => severityRank(a.severity) - severityRank(b.severity))
            .map((r) => (
              <li key={r.rule} style={{ padding: "6px 0", borderBottom: "1px solid #f0f0f0" }}>
                <span style={{ fontWeight: 600, color: SEVERITY_COLOR[r.severity] ?? "#333" }}>
                  {r.rule}
                </span>
                <span style={{ color: "#888", marginLeft: 8 }}>×{r.count}</span>
