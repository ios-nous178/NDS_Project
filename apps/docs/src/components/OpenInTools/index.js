...
// docs/_drafts/... 같은 root 상대 경로로 정규화
// metadata.source 예: "@site/../../docs/guide/design-principles.md"
function sourceToRepoPath(source) {
...
const PROMPT_TEMPLATE = (url) => `문서에 대한 몇 가지 질문을 하고 싶어. 다음 문서를 읽어줘: ${url}`;

const cache = new Map();

export default function OpenInTools() {
  const { siteConfig } = useDocusaurusContext();
  const { metadata } = useDoc();
  const repo = siteConfig.customFields?.repo;
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      ...
      setToast({ kind: "error", message: `복사 실패: ${err.message}` });
    }
  }, [urls]);
    ...
    <div className={styles.wrapper} ref={containerRef}>
      ...
      {open && (
        <div className={styles.popover} role="menu">
          <button type="button" className={styles.option} onClick={handleCopy} role="menuitem">
            <CopyIcon />
            <span>마크다운 복사</span>
          </button>
          <a
      ...
      {toast && (
        <div
          className={toast.kind === "error" ? `${styles.toast} ${styles.toastError}` : styles.toast}
          role="status"
        >
          {toast.message}
        </div>
