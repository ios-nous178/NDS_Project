...
/* ─── Logo asset URLs ─── */
/**
 * 프로젝트 로고는 `@nudge-design/assets` 가 SSOT 이며, Storybook 은 `src/files` 를
 ...
 * @nudge-design/assets 의 scalable SVG (getProjectLogo("nudge-eap", "koEnHorizontal")).
 * NudgeEAP 헤더는 내장 vector 로고를 기본 사용하고, 이 fixture 는 목업/푸터 fallback 용.
 */
const nudgeEapHeaderLogo = assetUrl(getProjectLogo("nudge-eap", "koEnHorizontal")?.filename);
const trostLogo = assetUrl(getProjectLogo("trost")?.filename);
const trostLogoMobile = assetUrl(getProjectLogo("trost", "mobile")?.filename);
const genietLogoPc = assetUrl(getProjectLogo("geniet", "pc")?.filename);
const genietLogoMobile = assetUrl(getProjectLogo("geniet", "mobile")?.filename);
const genietLogoFooter = assetUrl(getProjectLogo("geniet", "footer")?.filename);
const cashwalkVertical = assetUrl(getProjectLogo("cashwalk-biz", "vertical")?.filename);
/**
...
const cashwalkForBusinessHorizontal = assetUrl(
  getProjectLogo("cashwalk-biz", "horizontalSvg")?.filename,
);
 ...
 *
 * Admin 데스크톱 위주이지만 ProjectFixture 의 모바일 슬롯도 horizontal lockup 으로 통일.
 */
const cashwalkBiz: ProjectFixture = {
  logo: {
...
export function getProjectFixture(projectKey: string): ProjectFixture {
  return projectFixtures[projectKey] || projectFixtures["nudge-eap"];
}
