...
/**
 * Brands/Icons — 4개 브랜드(Geniet · Trost · Runmile · CashwalkBiz)의 전용 아이콘 카탈로그.
 * 그리드·검색·복사는 공유 컴포넌트(@nudge-design/catalog)와 단일 구현을 공유하고,
 * 데이터는 metadata/iconCatalog.json 의 brand 분류(또는 cashwalkBiz 큐레이션 섹션)에서 자동 분기.
 * 각 스토리가 `globals.brand` 로 테마를 건다.
 *
 * 정책: 공통 컴포넌트(AppFooter/BottomNav)에 `if (brand === …)` 분기를 박지 말고,
 * 사용처에서 명시적으로 import 해 전달. 브랜드 모드 작업 시 공용 아이콘보다 이쪽을 우선 사용.
 */
...
/* 브랜드별 기본 args — 각 스토리에 spread */
const GENIET_ARGS = { mode: "single-brand", brand: "geniet" } as const;
const TROST_ARGS = {
  mode: "single-brand",
  brand: "trost",
  searchPlaceholder: "이름으로 검색 (예: mental, testresult, link)",
} as const;
const RUNMILE_ARGS = { mode: "single-brand", brand: "runmile" } as const;
const CASHWALK_ARGS = {
  mode: "cashwalk-biz",
    ...
    docs: {
      description: {
        story: [
          ...
          "공용 아이콘과 별개로 Geniet 디자인 그대로 가져온 변종. 브랜드 모드(brand='geniet')에서 우선 사용.",
...
export const GenietBrandTint: Story = {
  name: "Geniet/Brand 색",
  globals: { brand: "geniet" },
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Geniet brand 컬러(`--semantic-icon-brand-default`) 로 렌더링. 브랜드 테마가 적용된 토큰을 따릅니다.",
      },
    },
  },
};
...
export const TrostBrandTint: Story = {
  name: "Trost/Brand 색 (cobalt)",
  globals: { brand: "trost" },
    ...
    docs: {
      description: {
        story: [
          ...
          "5 컬러 슬롯(black/white/gray600/gray800/orange500)은 시멘틱 토큰(`--semantic-icon-{strong,inverse,muted,normal,brand}-default`)으로 매핑.",
...
export const RunmileBrandTint: Story = {
  name: "Runmile/Brand 색 (orange500)",
  globals: { brand: "runmile" },
...
export const CashwalkBizBrandTint: Story = {
  name: "CashwalkBiz/Brand 색",
  globals: { brand: "cashwalk-biz" },
