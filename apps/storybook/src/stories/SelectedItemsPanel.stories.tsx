...
import { RegionRow, SelectedItemsPanel } from "@nudge-design/react";
import React, { useState } from "react";

const meta: Meta<typeof SelectedItemsPanel> = {
  title: "Components/SelectedItemsPanel",
    ...
    docs: {
      description: {
        component:
          ...
          "본문은 RegionRow 리스트 · 폼 · 테이블 등으로 swap 하는 슬롯. 캐포비 admin 의 다중 선택 결과 패널 패턴 (Figma 3828:1577).",
      },
    },
  },
};
  ...
  return (
    <div style={{ maxWidth: 664 }}>
      <SelectedItemsPanel
        title="선택한 지역"
        count={items.length}
        onAdd={() => setItems((prev) => [...prev, `추가 지역 ${prev.length + 1}`])}
        onClear={() => setItems([])}
...
export const Empty: Story = {
  render: () => (
    <div style={{ maxWidth: 664 }}>
      <SelectedItemsPanel title="선택한 지역" count={0} onAdd={() => {}} onClear={() => {}}>
        <div
          style={{
...
export const NoActions: Story = {
  render: () => (
    <div style={{ maxWidth: 664 }}>
      <SelectedItemsPanel title="선택한 지역" count={3} showActions={false}>
        {SAMPLE.map((label) => (
          <RegionRow key={label}>{label}</RegionRow>
        ))}
      </SelectedItemsPanel>
    </div>
  ),
