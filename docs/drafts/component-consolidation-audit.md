...
| 컴포넌트         | 도메인        | 기준1 (2+ 프로젝트)          | 기준2 (figmaNodeUrl) | html 미러 | 판정                                                                          |
| ---------------- | ------------- | -------------------------- | -------------------- | --------- | ----------------------------------------------------------------------------- |
...
| AppointmentCard  | 상담 예약     | NudgeEAP/Trost 상담 도메인 | **없음**             | 있음      | 보류 — 2개 프로젝트 사용이면 기준1 충족. 기준2(figma)·기준3(도메인) 재평가 필요 |
...
탐색 단계에서 "16개"로 보고됐으나 **실측은 2건** — 나머지는 프로젝트 셸 비대칭(4절)이었다.
...
| 차이                         | 건수 | 원인                                                                                                                                                                                                                                                                                |
| ---------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 가이드 ⊃ react(top-level)    | +21  | 프로젝트 셸 가이드 — ProjectChrome/ProjectHeader/ProjectFooter/ProjectBottomNav(합성 4) + 프로젝트별 AppBar/Footer/BottomNav/WebHeader/ScrollFab/Toast(17). 구현은 `packages/react/src/{project}/` 서브디렉토리( barrel 별도) 또는 html 전용(nds-project-chrome)이라 top-level export 집계에서 빠짐 |
| inventory ⊃ react(top-level) | +14  | 같은 프로젝트 셸 중 inventory 에 등재된 부분집합 (AppBar/WebHeader + 프로젝트별 12)                                                                                                                                                                                                     |
| react ⊃ guides               | +2   | FormSection · SelectionButton (3절)                                                                                                                                                                                                                                                 |
| react ⊃ inventory            | +8→0 | AddButton/Header/ProjectLogo/PageSizeSelect/InputGroup/SelectionButton/Sidebar — 이번 라운드에 `scripts/storybook-catalog-baseline.json` waiver 로 박제, 신규 누락은 게이트가 차단                                                                                                    |

결론: 수량 차이의 대부분은 "방치"가 아니라 **프로젝트 셸의 집계 표면 비대칭**이다. 단,
프로젝트 셸을 카탈로그(인벤토리·가이드·스토리)에서 어떤 단위로 셀지(합성 4개로 셀지, 프로젝트×종류로 셀지)
규칙이 없어서 표면마다 다르게 세고 있다 — 이 규칙을 정하면 3면 게이트로 강제할 수 있다.

## 5. 부수 발견

- `projects/cashwalk-biz/DESIGN.md` 부재 — 4개 프로젝트만 DESIGN.md 보유. 캐포비 토큰 의도 문서화 공백.
...
감사는 projects/·mockup-core/·apps/ grep 으로 거의 전부 "실사용 0"으로 판정했으나, **사용자가
MediaCard 는 Geniet 에서 확실히 쓰인다고 정정**. 원인: 이 레포의 `projects/*` 는 토큰 스캐폴드
(DESIGN.md)일 뿐 실제 프로젝트 앱이 아니다 — 진짜 소비처는 npm 으로 DS 를 받는 **별도 앱 레포**라
인-레포 grep 으로 안 잡힌다. 따라서 "0 usage → 삭제 안전" 추론은 **무효**. 제거/통합 후보는
실사용 여부를 사용자에게 확인한 뒤에만 손댄다.

**최종 결정(사용자)**: deprecate 단계를 거치지 않고 **즉시 하드 삭제**(`UserCard·CounselorCard·AppointmentCard`).
외부 소비자가 import 중이면 빌드가 깨지므로 changeset 에 BREAKING 명시 + Card 합성 마이그레이션 매핑을
가이드/ProductCard 가이드에 남김. (deprecate→1 minor 경유는 사용자가 불필요하다고 판단.) `MediaCard·ReviewCard`
는 범용성이 커서 `일반` 으로 재분류(유지). 진짜 keep 은 도메인 로직이 실재하는 **ProductCard** 와 범용
표시 카드 **MediaCard·ReviewCard**.

### 실행 결과

| 컴포넌트                           | 판정          | 처리                                                                                                                |
| ---------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------- |
| **VotePoll**                       | 제거          | 사용자 명시 삭제 + figmaNodeUrl 부재. 전 표면 제거. 설문은 LikertScale.                                              |
| **UserCard**                       | **제거**      | 순수 슬롯 배치 → `Card` 합성(Avatar+Title+Subtitle+Description+Metadata+Cta). BREAKING(changeset). 마이그레이션 매핑 보존. |
| **CounselorCard**                  | **제거**      | 순수 presentational → `Card.stories` CompoundCounselorCard 레시피. BREAKING.                                         |
| **AppointmentCard**                | **제거**      | 날짜파생+status/mode 상태머신 = 앱 로직(편입기준 위반). 앱 레이어로 이관, 시각은 `Card` 합성. BREAKING.              |
| **OrderSummaryCard**               | de-domain     | 범용 요약(라벨:값+합계). category `도메인`→`데이터`. 코드/API 무변경.                                                |
| **AttachmentItem · FloatingCtaBanner** | de-domain | category `도메인`→`일반(Display)`. 코드 무변경.                                                                      |
| **ContentViewer · MediaThumbnail** | de-domain     | 범용 프리미티브(안전-HTML 렌더러 / 이미지 표준화). 오태깅만 정정.                                                    |
| **MediaCard · ReviewCard**         | de-domain(유지) | 범용성 커서 `도메인`→`일반(Display)`. 컴포넌트 유지, 카탈로그 분류만.                                              |
| ProductCard                        | **유지**      | 가격/할인/적립/랭킹 = 커머스 도메인로직, Card 대체 불가. "Card" 네이밍 유지.                                         |
결론: 실제 제거 **4건**(VotePoll·UserCard·CounselorCard·AppointmentCard — 뒤 3건 BREAKING),
de-domain 재분류 **7건**(OrderSummaryCard·AttachmentItem·FloatingCtaBanner·ContentViewer·MediaThumbnail·
MediaCard·ReviewCard). 진짜 유지(도메인)는 **ProductCard** 등 소수. `도메인` 카테고리 20→10. Card 가이드의
합성 철학은 **신규** 도메인 카드 추가를 막는 기준으로 상시 활용 — ReviewCard 후속 클린업(빈 별 `#E0E0E0`
토큰화·figmaNodeUrl)은 별도 트랙.
