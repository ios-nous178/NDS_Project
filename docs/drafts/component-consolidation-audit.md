...
사용자 확인 결과: UserCard·CounselorCard·AppointmentCard·OrderSummaryCard 모두 **실사용 → 유지**.

### 실행 결과

| 컴포넌트                           | 판정      | 처리                                                                                                                    |
| ---------------------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------- |
| **VotePoll**                       | 제거      | 사용자 명시 삭제 지시 + figmaNodeUrl 부재. react/styles/html/가이드/스토리/docs/카탈로그 전부 제거. 설문은 LikertScale. |
| **AttachmentItem**                 | de-domain | category `도메인`→`일반`, storybookTitle `Domain`→`Display`. 코드 무변경(범용 파일 list-item).                          |
| **FloatingCtaBanner**              | de-domain | 동상 — figmaNodeUrl(91:3) 보유 정상 프리미티브, "도메인" 오태깅만 정정.                                                 |
| ProductCard                        | 유지      | 가격/할인/적립/랭킹 = 커머스 도메인로직, Card 대체 불가. "Card" 네이밍 유지(도메인 카드 패밀리 일관성).                 |
| ReviewCard                         | 유지      | half-star 알고리즘+verified+split 헤더. 네이밍 유지. (후속: 빈 별 `#E0E0E0` raw hex 토큰화·figmaNodeUrl 채우기)         |
| MediaCard                          | 유지      | **Geniet 실사용 확인**. (가이드상 "미디어 콘텐츠 카드" 합성과 동형이나 실사용이라 유지)                                 |
| UserCard / CounselorCard           | 유지      | Card 합성 가능하나 **실사용 확인** → 유지.                                                                              |
| AppointmentCard / OrderSummaryCard | 유지      | **실사용 확인** → 유지. (AppointmentCard 는 앱 로직 포함이나 실사용이라 존속)                                           |

결론: 이번 라운드 실제 제거는 **VotePoll 1건**, 재분류 2건(AttachmentItem·FloatingCtaBanner).
나머지 도메인 카드는 "Card 합성으로 대체 가능"하더라도 외부 실사용이 확인돼 유지. Card 가이드의
합성 철학은 **신규** 도메인 카드 추가를 막는 기준으로 활용(기존은 실사용 우선).
