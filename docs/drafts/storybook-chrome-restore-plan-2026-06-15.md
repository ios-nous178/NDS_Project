# Storybook 업데이트 플랜 — chrome 정리 이후 브랜드 chrome/사이드바 데모 복원 (2026-06-15)

> chrome 통합(Phase 1~4) 으로 **삭제된 스토리들의 대체 데모가 없어** Storybook 에 빈 자리가 생겼다.
> 브랜드 chrome·사이드바를 **목업 전용 html 웹컴포넌트(`nds-brand-*` / `nds-sidebar`)로** Storybook 에 다시 보이게 한다.
...
- **preview.ts 가 html 런타임을 등록하지 않는다** — tokens.css + `react/dist/styles.css` 만 import. `@nudge-design/html` 의존도 없음. → 지금 `<nds-brand-header>` 를 스토리에 써도 **custom element 미등록이라 빈 박스**로 뜬다. **이게 핵심 인프라 공백.**
- **html-in-Storybook 선례 없음** — 기존 스토리는 전부 react. `nds-*` 는 prose 언급만.
- **storybook-catalog 게이트는 `Components/*` 타이틀만 inventory 와 대조** (`check-storybook-catalog.mjs:46`). `Brands/*` 타이틀 스토리는 **면제** → 브랜드 chrome/사이드바 스토리는 inventory 재등록 불필요.
- 실행 중인 dev 서버(:6007)는 Phase 1 때 띄운 거라 **stale** 가능 — **재시작하면 삭제분이 반영**된다(우선 재시작해 진짜 현황 확인).
- ⚠ 동시 머지: `chrome-banner` 워크트리가 **EAPBanner → `nds-brand-chrome` `banner` 영역 일반화**(`renderBrandBanner` + `nds-brand-banner`)를 landing 중. 머지 후엔 **배너도 Storybook 데모 후보**가 된다 → 이 플랜은 머지 정착 후 착수.

## 결정 포인트 (착수 전 1개만 확정)

**브랜드 chrome/사이드바를 Storybook 에 어떻게 둘까?**

- **A. html 웹컴포넌트로 복원 (권장 · 사용자 의도)** — `nds-brand-*`/`nds-sidebar` 를 Storybook 에서 직접 렌더. 디자이너가 SSOT(목업이 실제 쓰는 것)를 그대로 본다. 인프라(런타임 등록) 필요.
...
1. `apps/storybook/package.json` 에 `@nudge-design/html: workspace:*` 추가 (+ `@nudge-design/assets` 도 brand 로고 data URI 가 런타임 주입이면 이미 html 가 의존하니 확인).
2. `apps/storybook/.storybook/preview.ts` 에 **html 런타임 등록** 추가: `import "@nudge-design/html";` (모든 `nds-*` custom element 를 `customElements.define`). 브랜드 chrome 스타일은 자기 render 가 inline `<style>` 주입 + `react/dist/styles.css`(=@nudge-design/styles, `.nds-*` 보유) + brand-themes decorator 의 `data-brand`/cssVars 로 커버 — **렌더 후 실측 확인**(특히 brand CSS 토큰).
3. **`NdsTag` 렌더 헬퍼** 1개 신설 (`apps/storybook/src/stories/_ndsTag.tsx` 등):
   - `<NdsTag tag="nds-brand-header" attrs={{ brand, surface, "active-key": ... }} />` → `React.createElement(tag, ...)` + ref 로 attribute set. TS 의 JSX.IntrinsicElements 미정의 회피.
...
8. (머지 후) `BrandBanner` — 기존 `BrandBanner.stories` 가 react `Banner` 라면, `nds-brand-banner`(chrome-banner 머지분) 데모도 추가 검토.

## Phase C — 사이드바 스토리 (+ 여닫기 데모, 사용자 요청)

9. `Sidebar.stories.tsx` (`Brands/Sidebar` 또는 `Brands/Admin/Sidebar`) — `<nds-sidebar brand="cashwalk-biz">` + `pattern:cashwalk-biz-admin-sidebar` ready-made(items/account/footer-actions JSON slot) 주입.
...
- **스타일 로딩** — 브랜드 chrome render 의 inline `<style>` + styles.css + brand cssVars 가 Storybook iframe 에서 다 맞물리는지 실측 필수(토큰 누락 시 색 빠짐).
- **브랜드 테마 decorator 와 brand= 속성 이중지정** — 스토리의 `brand` 속성과 전역 brand globals 가 어긋나지 않게(개요 페이지에서 여러 브랜드 동시 렌더 시 `data-brand` 스코프).
- **착수 타이밍** — `chrome-banner`/`assets-tailwind` 워크트리 머지가 끝난 뒤. banner 일반화·NudgeEAPLogo 제거가 brand 로고/배너 데모에 영향.
- **게이트** — `Brands/*` 면제 확인됨. 단 신규 react helper 파일은 eslint(rules-of-hooks 등) 통과 필요(BottomNav.stories 선례: hook 은 컴포넌트로 분리).

## 작업량 추정 (A안 기준)

- Phase A 인프라: 0.5일 (런타임 등록 + NdsTag 헬퍼 + 스타일 실측) — **불확실성 최대 구간**.
- Phase B 브랜드 chrome 3스토리: 0.5일.
