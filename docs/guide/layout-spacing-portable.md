...
| 의도 | px | 사용처 | DS 토큰(참고) |
| --- | --- | --- | --- |
| tight | **4** | Chip · Badge · 인라인 라벨 그룹 | `--gap-tight` |
| default ★ | **10** | 표준 컴포넌트 gap (가장 자주) | `--gap-default` |
| comfortable | **12** | 폼 필드 · 세그먼트 · 라벨↔필드 | `--gap-comfortable` |
| loose | **16** | 컴포넌트 ↔ 컴포넌트 | `--gap-loose` |
| wide | **24** | 큰 영역 ↔ 큰 영역, 섹션 구분 | `--gap-wide` |
...
| 사용처 | px | 비고 | DS 토큰(참고) |
| --- | --- | --- | --- |
| Chip · Badge 내부 | **8** | 작은 인라인 컨테이너 | `--inset-chip` |
| Input · 작은 컨테이너 | **12** | 입력 필드 padding | `--inset-input` |
| Card ★ | **16** | 표준 카드 padding | `--inset-card` |
| Card Large | **20** | 큰 카드 · 강조 박스 | `--inset-card-large` |
| Modal · 통계 박스 | **24** | Modal Body, KPI Box | `--inset-modal` |
...
이렇게 두면 본문 코드는 Nudge DS 와 동일하게 `padding: var(--inset-card)` / `gap: var(--gap-default)` 형태로 작성할 수 있고, 나중에 DS 로 옮길 때 변환 비용이 거의 없습니다.
