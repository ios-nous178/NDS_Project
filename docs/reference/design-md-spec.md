...
- **프로젝트 기반**: 기존 브랜딩 자료를 제공하면, 에이전트가 팔레트, 타이포그래피, 패턴을 추출합니다
...
"Project & Style"이라고도 합니다. 제품의 룩앤필에 대한 총체적 설명입니다. 이 섹션은 프로젝트 개성, 타겟 오디언스, UI가 불러일으켜야 하는 감정적 반응을 정의합니다. 특정 규칙이나 토큰이 정의되지 않은 경우 기본 맥락으로 사용됩니다.
...
```
## Components
- **Buttons**: 둥근 형태(8px), primary는 프로젝트 블루 채우기, secondary는 아웃라인 사용
