...
| `nested-card` | warn · invariant | 🟢 승격후보 | med | `ux:p2-card-justified` | waiver 필요(explicit) | 카드 안 카드 — 위계 모호. card-everything 과 동일 예외축. |
...
| `project-denied-button-color` | warn · project-policy | 🟡 예외선행 | med | `ux:p5-brand-cta-policy` | 차단 안전(policy 자동면제) | 프로젝트 금지 버튼색 — 브랜드 정책 예외. |
