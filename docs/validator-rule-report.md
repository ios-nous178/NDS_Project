...
- ✅ 승격완료(promoted, 차단 중): **13개** — `admin-sidebar-logo-not-component`, `missing-viewport-meta`, `consent-raw-checkbox`, `project-modal-single-button-fullwidth`, `region-as-chip`, `date-as-text-input`, `address-as-text-input`, `amount-as-text-input`, `amount-as-static-display`, `nds-custom-element-content-mutation`, `project-denied-button-color`, `nested-card`, `primary-cta-per-container`
- 🟢 승격후보(candidate): **18개** — 예외 없거나 정의됨, 바로 검토 가능
- 🟡 예외선행(context): **5개** — 예외 케이스 데이터화 후 승격
- ⚪ 현행유지(hold): **16개**

### 차단 안전성 (승격후보+예외선행 기준)

- ✅ 차단 안전: **22개** — 예외 없거나 auto/structural/policy 자동 면제. detect 배선만 하면 바로 error 승격 가능.
...
| 룰 | 현재 | 승격 | UX영향 | 예외 케이스 | 차단안전성 | 근거 |
| --- | --- | --- | --- | --- | --- | --- |
| `date-as-text-input` | error · model-guard | ✅ 승격완료 | high | — | 차단 가능(예외없음) | 날짜를 raw text 로 — DatePicker 미사용. deterministic(picker 사용 면제 가드). [승격 2026-06-26] |
| `address-as-text-input` | error · model-guard | ✅ 승격완료 | med | — | 차단 가능(예외없음) | 주소 raw text — AddressPicker 미사용. deterministic(email/URL/계층select 오탐 가드). [승격 2026-06-26] |
| `amount-as-static-display` | error · model-guard | ✅ 승격완료 | med | — | 차단 가능(예외없음) | 폼 값 자리에 정적 숫자 — 입력 불가. deterministic(폼 컨텍스트 한정 가드). [승격 2026-06-26] |
| `amount-as-text-input` | error · model-guard | ✅ 승격완료 | med | — | 차단 가능(예외없음) | 금액 raw text — AmountInput(콤마·단위·clamp) 미사용. deterministic. [승격 2026-06-26] |
| `consent-raw-checkbox` | error · model-guard | ✅ 승격완료 | med | — | 차단 가능(예외없음) | 약관 동의 raw checkbox — checkbox-group(필수/선택/전체동의) 미사용. deterministic. [승격 2026-06-26] |
...
| `admin-sidebar-logo-not-component` | error · model-guard | ✅ 승격완료 | med | — | 차단 가능(예외없음) | 사이드바 로고를 컴포넌트 대신 텍스트/수동img — 일관성. deterministic. [승격 2026-06-26] |
