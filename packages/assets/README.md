...
| ------------ | ------------------------ | --------------------------------------------------- |
| trost        | `default` (검정, 밝은 bg)| `brand/trost/logos/trost-logo.svg`                                    |
| trost        | `mobile`                 | `brand/trost/logos/trost-logo-mobile.webp`                            |
| trost        | `white` (흰색, 어두운 bg)| `brand/trost/logos/trost-logo-white.svg`                             |
...
- 물리 경로: `brand/{brand}/{type}/{id}.{ext}` (예: `brand/nudge-eap/images/rank/rank-01.png`). 공유 자산은 `shared/...`.
- 고해상도: 3x 는 **`{id}@3x.{ext}`** (`filename`=2x base, `filename3x`=3x → `srcset`).
- 로고 variant 키는 위 "파일명 컨벤션" 표 참조(`getBrandLogo(slug, variant)`).
...
- **예외**: 자산 id 는 **실제 Figma 노드명/파일명에 잠긴다**(traceability). upstream 이 오탈자여도 따라간다.
  - 예: runmile 일러스트 `alram`(`figmaNodeName: "img/alram"`, `alram.png`) 은 **의도적으로 `alram` 유지** — `illustration-metadata.ts` 에 `alram (alaRm 아님)` 으로 박제.
  - ⚠ 이 때문에 **아이콘은 `runmile-alarm`(수정), 에셋은 `alram`(잠금)** 으로 갈린다. 같은 개념이지만 의도된 분기이며, 새로 만드는 자산은 가능하면 올바른 철자를 upstream 에서부터 맞춘다.
