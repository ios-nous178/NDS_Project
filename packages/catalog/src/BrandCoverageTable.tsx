...
.nds-cov-brandsub{font-size:10px;font-weight:500;color:var(--semantic-text-muted-default,#9ca3af);margin-top:2px}
...
const TOOLTIP: Record<CoverageStatus, string> = {
  synced: "코드 ✓ + 해당 브랜드 Figma 가이드 ✓",
  code: "코드 있음, 이 브랜드 Figma 가이드 미정합",
  missing: "구현 없음",
};
...
export function BrandCoverageTable(props: BrandCoverageTableProps): React.JSX.Element {
  const { view, title = "Brand × Component Coverage", legend = true } = props;
  const { brands, summary, groups, chromeMatrix } = view;
  ...
  const stats = [
    ...
    ...brands.map((b) => ({
      label: `Figma — ${b.label}`,
      value: `${summary.figmaPerBrand[b.id] ?? 0} / ${summary.total}`,
      hint: "브랜드 가이드 정합",
    })),
  ];
  ...
  return (
    <div className="nds-cov-root">
      ...
      <div className="nds-cov-head">
        ...
        {legend ? (
          <p className="nds-cov-legend">
            5개 브랜드 × 2개 패키지(react/html) 구현 현황. <b>● synced</b> = 코드 + 해당 브랜드 Figma
            ...
            <tr>
              ...
              {brands.map((b) => (
                <th key={b.id} className="nds-cov-c" style={{ minWidth: 130 }}>
                  {b.label}
                  <div className="nds-cov-brandsub">React · HTML</div>
                </th>
              ))}
            </tr>
          </thead>
            ...
            {visibleGroups.map((g) => (
              <React.Fragment key={g.categoryKey}>
                <tr className="nds-cov-catrow">
                  <td colSpan={2 + brands.length}>{g.categoryLabel}</td>
                </tr>
                {g.rows.map((row) => (
                  <tr key={row.tds}>
                    <td>
                    ...
                    {row.cells.map((cell) => (
                      <td key={cell.brand} className="nds-cov-c">
      ...
      {chromeMatrix.length > 0 ? (
        <section>
          <p className="nds-cov-sub">브랜드 chrome (header / footer / nav)</p>
          <p className="nds-cov-subnote">
            packages/react/src/&#123;brand&#125;/ 폴더 기준 실제 브랜드 fork 가 있는 chrome 컴포넌트.
          </p>
