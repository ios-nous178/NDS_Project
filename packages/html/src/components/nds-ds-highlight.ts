...
export class NdsDsHighlight extends NdsElement {
  ...
  private _drawOverlay(rects: OverlayRect[]): void {
    ...
    rects.forEach((rect) => {
      ...
      border.style.cssText = `position:absolute;top:${rect.top - 3}px;left:${rect.left - 3}px;width:${rect.width + 6}px;height:${rect.height + 6}px;border:var(--stroke-focus) dashed ${rect.color};border-radius:8px;pointer-events:none;`;
