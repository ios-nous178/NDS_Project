/**
 * Project chrome wrappers for mockup-first HTML usage.
 *
 * 사용 패턴:
 *   <nds-project-header project="trost"  surface="web"></nds-project-header>
 *   <nds-project-header project="geniet" surface="web"></nds-project-header>
 *   <nds-project-footer project="geniet" surface="web"></nds-project-footer>
 *
 * Alias (project 분기 없이 바로 호출):
 *   <nds-trost-header></nds-trost-header>
 *   <nds-geniet-header surface="mobile"></nds-geniet-header>
 *
 * React 패키지의 project chrome (Trost/Geniet/NudgeEAP/CashwalkBiz WebHeader / AppBar)
 * 과 시각적으로 동등한 HTML 을 직접 렌더한다. 단순 wrapper 가 아니라 project 별
 * 풍부한 데이터를 들고 있으며, project-specific 마크업/스타일을 inline 으로 inject.
 */
...
type ProjectKey = "nudge-eap" | "trost" | "geniet" | "cashwalk-biz" | "runmile";
...
/**
 * 프로젝트 프로모/크로스셀 띠 배너 (desktop 헤더 상단). 프로젝트 무관 — 내용은 데이터로
 * 주입하고 색은 `--nds-project-banner-*` 슬롯으로 override (구 react TrostEAPBanner 의 후신).
 */
...
interface ProjectChrome {
  label: string;
  logo: ProjectLogo;
  mobileLogo?: ProjectLogo;
  footerLogo?: ProjectLogo;
  maxWidth: number;
  webMenu: MenuItem[];
  mobileTitle: string;
  ...
  /** 앱 하단 BottomNav 5탭. web 전용 project (cashwalk-biz) 는 미설정. */
  bottomNav?: BottomNavData;

  /** 데스크탑 헤더 상단 프로모/크로스셀 띠 배너. 프로젝트 무관, 토큰 override 가능. */
  banner?: BannerData;
}

/* ──────────────── Data ──────────────── */

const PROJECT_DATA: Record<ProjectKey, ProjectChrome> = {
  "nudge-eap": {
    label: "NudgeEAP",
  ...
  trost: {
    ...
    /* React TrostBottomNav (홈/심리상담/커뮤니티/멘탈케어/내공간) — active 색은 프로젝트
  ...
  runmile: {
     ...
     * 헤더/모바일 = coral default(PROJECT_LOGOS.runmile.default), footer = muted gray700
     * (PROJECT_LOGOS.runmile.muted). 외부 호스팅 없이도 깨지지 않게 inline. */
...
const PROJECT_KEYS = Object.keys(PROJECT_DATA) as ProjectKey[];

/* ──────────────── Utils ──────────────── */

function normalizeProject(value: string | null, fallback: ProjectKey): ProjectKey {
  return PROJECT_KEYS.includes(value as ProjectKey) ? (value as ProjectKey) : fallback;
}
...
/* 프로젝트 무관 프로모/크로스셀 띠 배너 (desktop 헤더 상단).
 * 색은 `--nds-project-banner-*` 슬롯 — default 는 현행 EAP 크로스셀 띠 값. 다른 프로젝트는
 * 데이터(banner)만 주입하고 필요 시 슬롯만 override (컴포넌트에 프로젝트 분기 없음). */
const PROJECT_BANNER_CSS = `
    .nds-project-banner {
      width: 100%;
      height: var(--nds-project-banner-height, 50px);
      background: var(--nds-project-banner-bg, #d5eafb);
      display: flex;
    ...
    .nds-project-banner__text {
      margin-right: 20px;
      font-size: 16px;
      line-height: 1.5;
      color: var(--nds-project-banner-text-color, #333);
      font-weight: 400;
    }
    .nds-project-banner__text > strong { font-weight: 700; }
    .nds-project-banner__cta {
      display: inline-flex;
      align-items: center;
      height: 34px;
      background: var(--nds-project-banner-cta-bg, #eaf5fd);
      border-radius: 8px;
      padding: 0 11px;
      text-decoration: none;
      box-sizing: border-box;
    }
    .nds-project-banner__cta-label {
      font-size: 14px;
      font-weight: 700;
      line-height: 1.43;
      color: var(--nds-project-banner-cta-color, #333);
    }
    .nds-project-banner__cta-label > span {
      color: var(--nds-project-banner-accent, #ff7a00);
    }
`;

/** 프로젝트 데이터의 banner 를 띠 배너 HTML 로 렌더 — desktop 헤더 첫 자식으로 삽입. */
function renderProjectBanner(banner: BannerData): string {
  const hasCta = !!(banner.ctaPrefix || banner.ctaAccent || banner.ctaSuffix);
  const cta = hasCta
    ? `<span class="nds-project-banner__cta"><p class="nds-project-banner__cta-label">${escapeHtml(banner.ctaPrefix ?? "")}${banner.ctaAccent ? `<span>${escapeHtml(banner.ctaAccent)}</span>` : ""}${escapeHtml(banner.ctaSuffix ?? "")}</p></span>`
    : "";
  return `${ensureStyle("nds-project-banner", PROJECT_BANNER_CSS)}<a class="nds-project-banner" href="${escapeAttr(banner.href)}" target="_blank" rel="noreferrer"><p class="nds-project-banner__text"><strong>${escapeHtml(banner.strong)}</strong>${escapeHtml(banner.text)}</p>${cta}</a>`;
}
 ...
 *   글자가 위 baseline 에 붙는다. project-chrome 안에서 한 번만 vertical center override. */
const WEBVIEW_FIX_ID = "nds-project-chrome-webview-fix";
const WEBVIEW_FIX_CSS = `
   ...
   * 무력화돼 타이틀이 top:50% 에만 걸려 아래로 떨어진다. nds-project-header 엘리먼트 셀렉터로
   * 특정성(0,3,1)을 올려 vertical center 가 이기게 한다. */
  nds-project-header .nds-header[data-variant="webview"] .nds-header__title {
    top: 50%;
  ...
  nds-project-header .nds-header[data-variant="webview"] {
    position: relative;
  }
`;
 ...
 * HTML 패키지는 다른 project (TROST/GENIET) 와 동일하게 inline SVG 로 self-contained
 ...
 * React 의 project BottomNav (TrostBottomNav/GenietBottomNav/NudgeEAPBottomNav/RunmileBottomNav)
 * 이 @nudge-design/icons 로 쓰는 것과 동일한 그래픽을, HTML 패키지는 다른 project chrome 과
 ...
 * ⚠️ url(#…) 참조가 있는 SVG (geniet-review clipPath) 는 다중 인라인 시
...
const BOTTOM_NAV_ICONS: Record<string, string> = {
  ...
  counsel: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g transform="translate(1.5 2.5)"><path d="M10.5 0C16.299 0 21 3.80558 21 8.5C21 13.1944 16.299 17 10.5 17C9.68157 17 8.88566 16.9207 8.12109 16.7773L4.65332 18.457C3.65727 18.9391 2.50011 18.214 2.5 17.1074V14.002C0.941964 12.519 0 10.5993 0 8.5C0 3.80558 4.70101 0 10.5 0Z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/><g><path d="M7.7 8.71997C7.7 9.27127 7.25263 9.71756 6.7 9.71756C6.14737 9.71756 5.7 9.27127 5.7 8.71997C5.7 8.16867 6.14737 7.72238 6.7 7.72238C7.25263 7.72238 7.7 8.16867 7.7 8.71997Z" fill="currentColor"/><path d="M11.5 8.71997C11.5 9.27127 11.0526 9.71756 10.5 9.71756C9.94738 9.71756 9.5 9.27127 9.5 8.71997C9.5 8.16867 9.94738 7.72238 10.5 7.72238C11.0526 7.72238 11.5 8.16867 11.5 8.71997Z" fill="currentColor"/><path d="M15.3 8.71997C15.3 9.27127 14.8522 9.71756 14.3009 9.71756C13.7495 9.71756 13.3 9.27127 13.3 8.71997C13.3 8.16867 13.7478 7.72238 14.3009 7.72238C14.854 7.72238 15.3 8.16867 15.3 8.71997Z" fill="currentColor"/></g></g></svg>`,
  ...
  "runmile-people": `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="14.8" cy="7.7" r="2.9" stroke="currentColor" stroke-width="1.3"/><circle cx="6.6" cy="7.2" r="2.5" stroke="currentColor" stroke-width="1.3"/><path d="M8.7 20.4C8.7 17.07 11.45 14.4 14.8 14.4C18.15 14.4 20.9 17.07 20.9 20.4" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/><path d="M1.9 19C1.9 16.4 3.6 14.1 6 13.4" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>`,
 ...
 * appDownload 버튼은 gray-100 bg + project text + project border 의 secondary 스타일. */
...
function renderNudgeEAPHeader(
  ...
  if (surface === "webview") {
    return renderWebviewHeader(project.mobileTitle);
  }
  ...
  if (surface === "mobile") {
    ...
    return `
      <nds-header variant="compact">
        <nds-header-logo slot="left" href="/">${renderLogoImg(project.logo, assetBaseUrl, { height: 28 })}</nds-header-logo>
        <nds-header-actions slot="right">
          <nds-header-auth-button label="${escapeAttr(project.authLabel ?? "로그인")}"></nds-header-auth-button>
        </nds-header-actions>
      </nds-header>
    `;
  }
  ...
  const styleId = "nds-project-chrome-nudge-eap-web";
  const css = `
    .nds-project-nudge-eap-web {
      display: block;
      width: 100%;
    ...
    .nds-project-nudge-eap-web__inner {
      max-width: var(--nds-nudge-eap-max-width, ${project.maxWidth}px);
      margin: 0 auto;
      height: 80px;
      display: flex;
      align-items: center;
    ...
    .nds-project-nudge-eap-web__logo {
      display: inline-flex;
      align-items: center;
      flex-shrink: 0;
      text-decoration: none;
    }
    .nds-project-nudge-eap-web__menu {
      position: absolute;
      left: 50%;
    ...
    .nds-project-nudge-eap-web__menu-item {
      display: inline-flex;
      align-items: center;
      height: 100%;
    ...
    .nds-project-nudge-eap-web__actions {
      margin-left: auto;
      display: inline-flex;
      align-items: center;
      gap: ${spacing[8]}px;
    ...
    .nds-project-nudge-eap-web__app-dl {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      height: 40px;
    ...
    .nds-project-nudge-eap-web__auth {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      height: 40px;
  ...
  const logoHref = "/";
  const logo = renderLogoImg(project.logo, assetBaseUrl, { height: 32 });
  const menu = project.webMenu
    .map(
      (item) =>
        `<a class="nds-project-nudge-eap-web__menu-item" href="${escapeAttr(item.href)}" data-key="${escapeAttr(item.key)}" ${item.key === activeKey ? 'data-active="true"' : ""}>${escapeHtml(item.label)}</a>`,
    )
    .join("");
  const appDl = project.nudgeEap
    ? `<a class="nds-project-nudge-eap-web__app-dl" href="${escapeAttr(project.nudgeEap.appDownloadHref)}">${escapeHtml(project.nudgeEap.appDownloadLabel)}</a>`
    : "";

  return `
    ${ensureStyle(styleId, css)}
    <header class="nds-project-nudge-eap-web" data-slot="root">
      <div class="nds-project-nudge-eap-web__inner">
        <a class="nds-project-nudge-eap-web__logo" href="${escapeAttr(logoHref)}">${logo}</a>
        <nav class="nds-project-nudge-eap-web__menu">${menu}</nav>
        <div class="nds-project-nudge-eap-web__actions">
          ${appDl}
          <a class="nds-project-nudge-eap-web__auth" href="#">${escapeHtml(project.authLabel ?? "로그인")}</a>
        </div>
      </div>
    </header>
  `;
...
function renderGenietHeader(
  ...
  const styleId = "nds-project-chrome-geniet";
  const css = `
    /* ── Geniet root ── */
    .nds-project-geniet {
      display: block;
    ...
    /* ── Desktop ── */
    .nds-project-geniet--desktop {
      padding-top: ${g.pcTopPadding}px;
    }
    .nds-project-geniet__search-header {
      display: flex;
      align-items: center;
      justify-content: space-between;
      width: 100%;
      max-width: ${project.maxWidth}px;
    ...
    .nds-project-geniet__menu-header {
      display: flex;
      align-items: stretch;
      width: 100%;
      max-width: ${project.maxWidth}px;
    ...
    .nds-project-geniet__logo {
      display: inline-flex;
      flex-shrink: 0;
      text-decoration: none;
    }
    .nds-project-geniet__search-frame {
      display: flex;
      align-items: center;
      gap: ${spacing[24]}px;
      flex-shrink: 0;
    }
    .nds-project-geniet__search-input {
      position: relative;
      display: flex;
      align-items: center;
    ...
    .nds-project-geniet__search-input input {
      all: unset;
      width: 100%;
    ...
    .nds-project-geniet__search-input input::placeholder {
      color: ${cv.textRole.muted};
    }
    .nds-project-geniet__search-icon {
      position: absolute;
      right: ${spacing[16]}px;
      top: 50%;
    ...
    .nds-project-geniet__trending-rank {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      width: 18px;
    ...
    .nds-project-geniet__trending-keyword {
      font-weight: ${fontWeight.bold};
      color: ${cv.textRole.strong};
      white-space: nowrap;
    }
    .nds-project-geniet__trending-stamp {
      color: ${cv.textRole.muted};
    }
    .nds-project-geniet__login-area {
      display: flex;
      align-items: center;
      gap: ${spacing[12]}px;
      flex-shrink: 0;
    }
    .nds-project-geniet__action-btn {
      all: unset;
      display: inline-flex;
    ...
    .nds-project-geniet__action-btn:hover {
      color: ${cv.textRole.normal};
    }
    .nds-project-geniet__action-btn-icon {
      display: flex;
      align-items: center;
      justify-content: center;
      width: 28px;
    ...
    .nds-project-geniet__action-btn-label {
      font-size: 11px;
      line-height: 14px;
      font-weight: ${fontWeight.medium};
      white-space: nowrap;
    }
    .nds-project-geniet__action-divider {
      width: 1px;
      height: 24px;
      background: ${cv.borderRole.subtle};
      flex-shrink: 0;
    }
    .nds-project-geniet__category {
      display: inline-flex;
      align-items: center;
      gap: ${spacing[10]}px;
      height: 100%;
    ...
    .nds-project-geniet__gnb-item:hover {
      color: ${cv.textRole.brand};
    }
    ...
    .nds-project-geniet__cta-pill[data-tone="tinted"] {
      background: #e9f7ff;
      color: #0093f1;
    }
    .nds-project-geniet__cta-pill[data-tone="filled"] {
      background: ${cv.fill.brand};
      color: ${cv.textRole.inverse};
    }

    /* ── Mobile ── */
    .nds-project-geniet--mobile {
    ...
    .nds-project-geniet__mo-right {
      display: inline-flex;
      align-items: center;
      gap: ${spacing[14]}px;
    }
    .nds-project-geniet__point-chip {
      display: inline-flex;
      align-items: center;
      gap: ${spacing[2]}px;
      height: 30px;
    ...
    .nds-project-geniet__mo-hamburger {
      all: unset;
      display: inline-flex;
      align-items: center;
      justify-content: center;
    ...
    .nds-project-geniet__mo-search input {
      all: unset;
      width: 100%;
      font-size: ${typeScale.body2.fontSize}px;
      line-height: ${typeScale.body2.lineHeight}px;
    ...
    .nds-project-geniet__mo-search input::placeholder {
      color: ${cv.textRole.muted};
    }
    .nds-project-geniet__mo-search-icon {
      position: absolute;
      right: ${spacing[12]}px;
      top: 50%;
      transform: translateY(-50%);
  ...
  if (surface === "mobile") {
    const logoImg = project.mobileLogo ?? project.logo;
    return `
      ${ensureStyle(styleId, css)}
      <header class="nds-project-geniet nds-project-geniet--mobile" data-slot="root">
        <div class="nds-project-geniet__mo-row1">
          <a class="nds-project-geniet__logo" href="/">${renderLogoImg(logoImg, assetBaseUrl)}</a>
          <div class="nds-project-geniet__mo-right">
            <a class="nds-project-geniet__point-chip" href="${escapeAttr(g.mobilePointHref)}" aria-label="포인트 ${escapeAttr(g.mobilePointAmount)}">
              <span style="display:inline-flex">${GENIET_ICONS.gpoint}</span>
              <span>${escapeHtml(g.mobilePointAmount)}</span>
            </a>
            <button type="button" class="nds-project-geniet__mo-user" aria-label="사용자">${GENIET_ICONS.user}</button>
          </div>
        </div>
        <div class="nds-project-geniet__mo-row2">
          <button type="button" class="nds-project-geniet__mo-hamburger" aria-label="음식 카테고리">${GENIET_ICONS.menu}</button>
          <div class="nds-project-geniet__mo-search">
            <input type="text" placeholder="${escapeAttr(g.mobileSearchPlaceholder)}" autocomplete="off" />
            <span class="nds-project-geniet__mo-search-icon" role="button" aria-label="검색" style="width:20px;height:20px;">${GENIET_ICONS.search.replace('width="24" height="24"', 'width="20" height="20"')}</span>
          </div>
        </div>
      </header>
    `;
  ...
  const trendingHtml = trendingTop
    ? `<div class="nds-project-geniet__trending">
         <span class="nds-project-geniet__trending-rank">${trendingTop.rank}</span>
         <span class="nds-project-geniet__trending-keyword">${escapeHtml(trendingTop.keyword)}</span>
         <span class="nds-project-geniet__trending-stamp">${escapeHtml(g.trendingTimestamp)}</span>
       </div>`
    : "";
  ...
  const actionButtonsHtml = g.actionButtons
    .map((action) => {
      ...
      const divider = action.dividerBefore
        ? `<span class="nds-project-geniet__action-divider" aria-hidden="true"></span>`
        : "";
      return `${divider}<a class="nds-project-geniet__action-btn" href="${escapeAttr(action.href ?? "#")}">
        <span class="nds-project-geniet__action-btn-icon">${icon}</span>
        <span class="nds-project-geniet__action-btn-label">${escapeHtml(action.label)}</span>
      </a>`;
    })
    .join("");

  const gnbHtml = project.webMenu
    .map(
      (item) =>
        `<a class="nds-project-geniet__gnb-item" href="${escapeAttr(item.href)}" data-key="${escapeAttr(item.key)}" ${item.key === activeKey ? 'data-active="true"' : ""}>${escapeHtml(item.label)}</a>`,
    )
    .join("");
  ...
  const ctaHtml = g.ctaButtons
    .map((cta) => {
      ...
      return `<a class="nds-project-geniet__cta-pill" data-tone="${cta.tone}" href="${escapeAttr(cta.href ?? "#")}">
  ...
  return `
    ${ensureStyle(styleId, css)}
    <header class="nds-project-geniet nds-project-geniet--desktop" data-slot="root">
      <div class="nds-project-geniet__search-header" data-slot="search-header">
        <a class="nds-project-geniet__logo" href="/">${renderLogoImg(project.logo, assetBaseUrl)}</a>
        <div class="nds-project-geniet__search-frame">
          <div class="nds-project-geniet__search-input">
            <input type="text" placeholder="${escapeAttr(g.searchPlaceholder)}" autocomplete="off" />
            <span class="nds-project-geniet__search-icon" role="button" aria-label="검색">${GENIET_ICONS.search}</span>
          </div>
          ${trendingHtml}
        </div>
        <div class="nds-project-geniet__login-area" data-slot="login-area">${actionButtonsHtml}</div>
      </div>
      <div class="nds-project-geniet__menu-header-rail" data-slot="menu-header-rail">
        <nav class="nds-project-geniet__menu-header" data-slot="menu-header">
          <a class="nds-project-geniet__category" href="${escapeAttr(g.categoryHref)}">
            ${GENIET_ICONS.menu}<span>${escapeHtml(g.categoryLabel)}</span>
          </a>
          <div class="nds-project-geniet__gnb">${gnbHtml}</div>
          <div class="nds-project-geniet__cta-group">${ctaHtml}</div>
        </nav>
      </div>
    </header>
  `;
 ...
 *   1) 띠 배너 (50h) — 프로젝트 무관 `banner` 데이터 → renderProjectBanner (다른 프로젝트도 재사용)
 *   2) Utility Header (logo + search form + login + app download button)
 *   3) Tab Navigation (70h, tabs with active underline)
 *
 * 모두 PROJECT_DATA[trost] 데이터로 구동 (목업 셸이라 호스트 앱 주입 없이 1벌로 렌더). */
...
function renderTrostHeader(
  ...
  const t = project.trost;
  if (!t) return "";

  /* Trost mobile/webview 공용 시멘틱 토큰 (data-project="trost" cascade + hex fallback). */
  ...
  // 트로스트 project accent(텍스트 orange #FF9D00) — N 뱃지 등 project 강조. (구 raw #ff7a00 정합)
  const TC_TEXT_PROJECT = "var(--semantic-text-brand-default, #FF9D00)";

  if (surface === "webview") {
    /* React TrostAppBar variant="webview" 의 기본 케이스 (sub / app=trost):
     * 뒤로 chevron + 중앙 타이틀 + 우측 알림 벨, 44h. (Storybook "sub — 뒤로+타이틀+알림") */
    const styleId = "nds-project-chrome-trost-webview";
    const css = `
      .nds-project-trost-webview {
        position: relative;
        display: flex;
        align-items: center;
        width: 100%;
      ...
      .nds-project-trost-webview__title {
        position: absolute;
        left: 50%;
        top: 50%;
        transform: translate(-50%, -50%);
      ...
      .nds-project-trost-webview__right {
        margin-left: auto;
        display: inline-flex;
        align-items: center;
        gap: 16px;
      ...
      .nds-project-trost-webview__action {
        all: unset;
        display: inline-flex;
        align-items: center;
        justify-content: center;
    ...
    `;
    const wvTitle = t.webviewTitle || project.mobileTitle;
    return `
      ${ensureStyle(styleId, css)}
      <header class="nds-project-trost-webview" data-slot="root" data-level="sub" data-app="trost">
        <button type="button" class="nds-project-trost-webview__back" aria-label="뒤로">${TROST_CHROME_ICONS.back}</button>
        <h1 class="nds-project-trost-webview__title">${escapeHtml(wvTitle)}</h1>
        <div class="nds-project-trost-webview__right">
          <button type="button" class="nds-project-trost-webview__action" aria-label="알림">${TROST_CHROME_ICONS.bell}</button>
        </div>
      </header>
    `;
  }
  ...
  if (surface === "mobile") {
    ...
    const styleId = "nds-project-chrome-trost-mobile";
    const logoImg = project.mobileLogo ?? project.logo;
    const css = `
      .nds-project-trost-mobile {
        display: flex;
        flex-direction: column;
        width: 100%;
        box-sizing: border-box;
      ...
      .nds-project-trost-mobile__logo {
        display: inline-flex;
        flex-shrink: 0;
        line-height: 0;
        text-decoration: none;
      }
      .nds-project-trost-mobile__right {
        display: inline-flex;
        align-items: center;
        gap: 10px;
        flex-shrink: 0;
      }
      .nds-project-trost-mobile__point {
        display: inline-flex;
        align-items: center;
        gap: 4px;
        color: ${TC_TEXT_STRONG};
      ...
      .nds-project-trost-mobile__coin { display: inline-flex; line-height: 0; }
      .nds-project-trost-mobile__bell {
        all: unset;
        display: inline-flex;
        align-items: center;
        justify-content: center;
      ...
      .nds-project-trost-mobile__row2 {
        display: flex;
        align-items: center;
        height: 52px;
        flex-shrink: 0;
      ...
      .nds-project-trost-mobile__search {
        position: relative;
        display: flex;
        align-items: center;
        width: 100%;
      ...
      .nds-project-trost-mobile__search:focus-within { border-color: ${TC_BORDER_NORMAL}; }
      .nds-project-trost-mobile__search input {
        width: 100%;
        border: none;
        outline: none;
      ...
      .nds-project-trost-mobile__search input::placeholder { color: ${TC_TEXT_MUTED}; }
      .nds-project-trost-mobile__search-icon {
        position: absolute;
        right: 12px;
        top: 50%;
        transform: translateY(-50%);
    ...
    return `
      ${ensureStyle(styleId, css)}
      <header class="nds-project-trost-mobile" data-slot="root">
        <div class="nds-project-trost-mobile__row1">
          <a class="nds-project-trost-mobile__logo" href="/">${renderLogoImg(logoImg, assetBaseUrl, { height: 28 })}</a>
          <div class="nds-project-trost-mobile__right">
            <a class="nds-project-trost-mobile__point" href="/point" aria-label="포인트 ${escapeAttr(t.mobilePointAmount)}P">
              <span class="nds-project-trost-mobile__coin">${TROST_CHROME_ICONS.coin}</span>
              <span>${escapeHtml(t.mobilePointAmount)} P</span>
            </a>
            <button type="button" class="nds-project-trost-mobile__bell" aria-label="알림">${TROST_CHROME_ICONS.bell}</button>
          </div>
        </div>
        <div class="nds-project-trost-mobile__row2">
          <div class="nds-project-trost-mobile__search">
            <input type="text" placeholder="${escapeAttr(t.mobileSearchPlaceholder)}" autocomplete="off" />
            <span class="nds-project-trost-mobile__search-icon" role="button" aria-label="검색">${TROST_CHROME_ICONS.search}</span>
          </div>
        </div>
      </header>
    `;
  ...
  const styleId = "nds-project-chrome-trost-web";
  const css = `
    /* ── Trost Web header (3-tier) ── */
    .nds-project-trost-web {
      display: block;
      width: 100%;
      background: ${TC_BG};
      box-sizing: border-box;
    ...
    .nds-project-trost-web__utility-left {
      display: flex;
      justify-content: flex-start;
      align-items: center;
    }
    .nds-project-trost-web__utility-logo {
      width: 140px;
      height: 36px;
      padding-right: 50px;
      display: inline-flex;
    ...
    .nds-project-trost-web__utility-right {
      display: flex;
    ...
    .nds-project-trost-web__search input::placeholder {
      color: ${TC_TEXT_NORMAL};
      font-weight: 400;
    }
    .nds-project-trost-web__search-submit {
      position: absolute;
      right: 16px;
      top: 14px;
      cursor: pointer;
    ...
    .nds-project-trost-web__login {
      display: inline-flex;
      align-items: center;
    }
    .nds-project-trost-web__login-btn {
      margin-right: 20px;
      font-size: 16px;
      line-height: 24px;
      color: ${TC_TEXT_STRONG};
    ...
    .nds-project-trost-web__login-btn--partner { margin-right: 32px; }

    /* App download button */
    .nds-project-trost-web__app-dl {
      height: 44px;
      padding: 11px 16px;
      border-radius: 12px;
    ...
    .nds-project-trost-web__tabnav-list {
      position: absolute;
      left: 50%;
      transform: translateX(-50%);
      display: flex;
    ...
    .nds-project-trost-web__tabnav-link {
      height: 100%;
      display: flex;
      align-items: center;
    ...
    .nds-project-trost-web__tabnav-new {
      display: inline-flex;
      margin-left: 4px;
      padding: 0 6px;
      height: 18px;
      background: ${TC_TEXT_PROJECT};
      color: ${TC_BG};
      font-size: 11px;
      line-height: 18px;
      font-weight: 700;
  ...
  const tabsHtml = t.tabs
    .map(
      (tab) =>
        `<li><a class="nds-project-trost-web__tabnav-link" href="${escapeAttr(tab.href)}" ${tab.href === t.activeTab ? 'data-active="true"' : ""}>${escapeHtml(tab.name)}${tab.isNew ? `<span class="nds-project-trost-web__tabnav-new">N</span>` : ""}</a></li>`,
    )
    .join("");

  return `
    ${ensureStyle(styleId, css)}
    <header class="nds-project-trost-web" data-slot="root">
      ${project.banner ? renderProjectBanner(project.banner) : ""}
      <section class="nds-project-trost-web__utility">
        <div class="nds-project-trost-web__utility-inner">
          <div class="nds-project-trost-web__utility-left">
            <a class="nds-project-trost-web__utility-logo" href="/" aria-label="Trost">${renderLogoImg(project.logo, assetBaseUrl)}</a>
            <form class="nds-project-trost-web__search" onsubmit="return false">
              <input type="text" placeholder="${escapeAttr(t.searchPlaceholder)}" autocomplete="off" />
              <button type="submit" class="nds-project-trost-web__search-submit" aria-label="검색" style="color:#333">${TROST_ICONS.search}</button>
            </form>
          </div>
          <div class="nds-project-trost-web__utility-right">
            <div class="nds-project-trost-web__login">
              <button type="button" class="nds-project-trost-web__login-btn">${escapeHtml(project.authLabel ?? "로그인")}</button>
              ${t.partnerSignupLabel ? `<button type="button" class="nds-project-trost-web__login-btn nds-project-trost-web__login-btn--partner">${escapeHtml(t.partnerSignupLabel)}</button>` : ""}
            </div>
            <button type="button" class="nds-project-trost-web__app-dl">${escapeHtml(t.appDownloadLabel)}</button>
          </div>
        </div>
      </section>
      <nav class="nds-project-trost-web__tabnav">
        <div class="nds-project-trost-web__tabnav-inner">
          <ul class="nds-project-trost-web__tabnav-list">${tabsHtml}</ul>
        </div>
      </nav>
    </header>
  `;
...
function renderCashwalkBizHeader(
  ...
  const c = project["cashwalk-biz"];

  if (surface === "webview") {
    return renderWebviewHeader(project.mobileTitle);
  }

  const styleId = "nds-project-chrome-cashwalk-biz";
  const css = `
    .nds-project-cashwalk-biz {
      display: block;
      width: 100%;
      background: ${cv.surface.default};
      border-bottom: var(--stroke-default) solid ${cv.borderRole.subtle};
    ...
    .nds-project-cashwalk-biz__inner--mobile {
      height: ${c?.mobileHeight ?? 56}px;
      padding: 0 ${spacing[20]}px;
      justify-content: space-between;
    }
    .nds-project-cashwalk-biz__logo {
      display: inline-flex;
      align-items: center;
      flex-shrink: 0;
      text-decoration: none;
    }
    .nds-project-cashwalk-biz__menu {
      display: flex;
      align-items: center;
      gap: ${spacing[8]}px;
      flex: 1;
    ...
    .nds-project-cashwalk-biz__menu-item,
    .nds-project-cashwalk-biz__menu-item:visited,
    .nds-project-cashwalk-biz__menu-item:hover {
      display: inline-flex;
      align-items: center;
      height: 44px;
      padding: 0 ${spacing[12]}px;
    ...
    .nds-project-cashwalk-biz__actions {
      display: inline-flex;
      align-items: center;
      gap: ${spacing[16]}px;
      flex-shrink: 0;
    }
    .nds-project-cashwalk-biz__auth {
      display: inline-flex;
      align-items: center;
      height: 36px;
    ...
    .nds-project-cashwalk-biz__hamburger {
      all: unset;
      display: inline-flex;
      align-items: center;
      justify-content: center;
  ...
  if (surface === "mobile") {
    ...
    return `
      ${ensureStyle(styleId, css)}
      <header class="nds-project-cashwalk-biz" data-slot="root">
        <div class="nds-project-cashwalk-biz__inner nds-project-cashwalk-biz__inner--mobile">
          <a class="nds-project-cashwalk-biz__logo" href="/">${renderLogoImg(logoImg, assetBaseUrl)}</a>
          <button type="button" class="nds-project-cashwalk-biz__hamburger" aria-label="메뉴">${TROST_ICONS.hamburger}</button>
        </div>
      </header>
    `;
  }

  const menuHtml = project.webMenu
    .map(
      (item) =>
        `<a class="nds-project-cashwalk-biz__menu-item" href="${escapeAttr(item.href)}" data-key="${escapeAttr(item.key)}" ${item.key === activeKey ? 'data-active="true"' : ""}>${escapeHtml(item.label)}</a>`,
    )
    .join("");

  const ctaHtml = c?.primaryCta
    ? `<a class="nds-project-cashwalk-biz__primary-cta" href="${escapeAttr(c.primaryCta.href)}">${escapeHtml(c.primaryCta.label)}</a>`
    : "";

  return `
    ${ensureStyle(styleId, css)}
    <header class="nds-project-cashwalk-biz" data-slot="root">
      <div class="nds-project-cashwalk-biz__inner">
        <a class="nds-project-cashwalk-biz__logo" href="/">${renderLogoImg(project.logo, assetBaseUrl)}</a>
        <nav class="nds-project-cashwalk-biz__menu">${menuHtml}</nav>
        <div class="nds-project-cashwalk-biz__actions">
          <a class="nds-project-cashwalk-biz__auth" href="#">${escapeHtml(project.authLabel ?? "로그인")}</a>
          ${ctaHtml}
        </div>
      </div>
    </header>
...
function renderRunmileHeader(
  ...
  if (surface === "mobile") {
    ...
    const styleId = "nds-project-chrome-runmile-mobile";
    const css = `
      .nds-project-runmile-mobile {
        position: relative;
        width: 100%;
        height: 52px;
        background: ${C_WHITE};
      ...
      .nds-project-runmile-mobile__logo {
        position: absolute;
        left: 50%;
        top: 50%;
        transform: translate(-50%, -50%);
    ...
    return `
      ${ensureStyle(styleId, css)}
      <header class="nds-project-runmile-mobile" data-slot="root">
        <a class="nds-project-runmile-mobile__logo" href="/">${renderLogoImg(project.mobileLogo ?? project.logo, assetBaseUrl)}</a>
      </header>
    `;
  }

  /* desktop — 1단 80h */
  const styleId = "nds-project-chrome-runmile-web";
  const css = `
    .nds-project-runmile-web {
      display: block;
      width: 100%;
      height: 80px;
      background: ${C_WHITE};
    ...
    .nds-project-runmile-web__logo span {
      font-size: 22px;
      font-weight: 800;
      color: ${C_CORAL_TEXT};
      line-height: 1;
    }
    .nds-project-runmile-web__nav {
      display: flex;
      align-items: center;
      gap: 36px;
      flex-shrink: 0;
    }
    .nds-project-runmile-web__nav-item {
      font-size: 18px;
      line-height: 24px;
      font-weight: 700;
      color: ${C_BLACK};
    ...
    .nds-project-runmile-web__nav-item[data-active="true"] {
      color: ${C_CORAL_TEXT};
    }
    .nds-project-runmile-web__search {
      position: absolute;
      left: 50%;
      top: 50%;
      transform: translate(-50%, -50%);
    ...
    .nds-project-runmile-web__search input {
      flex: 1;
      min-width: 0;
      border: none;
      outline: none;
    ...
    .nds-project-runmile-web__search input::placeholder {
      color: ${C_PLACEHOLDER};
    }
    .nds-project-runmile-web__search-btn {
      flex-shrink: 0;
      width: 24px;
      height: 24px;
      padding: 0;
    ...
    .nds-project-runmile-web__actions {
      margin-left: auto;
      display: flex;
      align-items: center;
      gap: 10px;
    ...
    .nds-project-runmile-web__action {
      display: inline-flex;
      flex-direction: column;
      align-items: center;
      gap: 4px;
    ...
    .nds-project-runmile-web__action-icon {
      width: 28px;
      height: 28px;
      line-height: 0;
      color: ${C_ICON_GRAY};
    }
    .nds-project-runmile-web__action-label {
      font-size: 14px;
      line-height: 20px;
      font-weight: 500;
      color: ${C_LABEL};
  ...
  const navHtml = project.webMenu
    .map(
      (item) =>
        `<a class="nds-project-runmile-web__nav-item" href="${escapeAttr(item.href)}" data-key="${escapeAttr(item.key)}" ${item.key === activeKey ? 'data-active="true"' : ""}>${escapeHtml(item.label)}</a>`,
    )
    .join("");

  return `
    ${ensureStyle(styleId, css)}
    <header class="nds-project-runmile-web" data-slot="root">
      <div class="nds-project-runmile-web__inner">
        <a class="nds-project-runmile-web__logo" href="/">${renderLogoImg(project.logo, assetBaseUrl)}</a>
        <nav class="nds-project-runmile-web__nav">${navHtml}</nav>
        <div class="nds-project-runmile-web__search">
          <input type="text" placeholder="${escapeAttr(r.searchPlaceholder)}" autocomplete="off" />
          <button type="button" class="nds-project-runmile-web__search-btn" aria-label="검색">${RUNMILE_ICONS.search}</button>
        </div>
        <div class="nds-project-runmile-web__actions">
          <a class="nds-project-runmile-web__action" href="#" aria-label="${escapeAttr(r.chatLabel)}">
            <span class="nds-project-runmile-web__action-icon">${RUNMILE_ICONS.chat}</span>
            <span class="nds-project-runmile-web__action-label">${escapeHtml(r.chatLabel)}</span>
          </a>
          <a class="nds-project-runmile-web__action" href="#" aria-label="${escapeAttr(r.loginLabel)}">
            <span class="nds-project-runmile-web__action-icon">${RUNMILE_ICONS.login}</span>
            <span class="nds-project-runmile-web__action-label">${escapeHtml(r.loginLabel)}</span>
          </a>
        </div>
      </div>
    </header>
...
function renderHeader(
  ...
  switch (projectKey) {
    case "geniet":
      return renderGenietHeader(project, surface, activeKey, assetBaseUrl);
    case "trost":
      return renderTrostHeader(project, surface, activeKey, assetBaseUrl);
    case "cashwalk-biz":
      return renderCashwalkBizHeader(project, surface, activeKey, assetBaseUrl);
    case "runmile":
      return renderRunmileHeader(project, surface, activeKey, assetBaseUrl);
    case "nudge-eap":
    default:
      return renderNudgeEAPHeader(project, surface, activeKey, assetBaseUrl);
  }
}
 ...
 *   default (파란 밑줄) 로 떨어지는 문제를 project-chrome 자체 안에서 막는다. */
const FOOTER_LINK_RESET_ID = "nds-project-chrome-footer-link-reset";
const FOOTER_LINK_RESET_CSS = `
  :where(nds-project-footer) .nds-footer__link,
  :where(nds-project-footer) .nds-footer__link:visited,
  :where(nds-project-footer) .nds-footer__link:hover {
    color: var(--nds-footer-color, inherit);
    text-decoration: none;
  }
  :where(nds-project-footer) .nds-footer__link[data-bold="true"] {
    font-weight: 700;
  }
  :where(nds-project-footer) .nds-footer__links {
    display: flex;
    flex-wrap: wrap;
    gap: 12px 16px;
    margin-bottom: 12px;
  }
  :where(nds-project-footer) .nds-footer__extra {
   ...
   * 렌더되는데, project-logos 원본이 크면 footer 가 망가진다. 모든 project 동일 톤 (height 32) 로 강제. */
  :where(nds-project-footer) .nds-footer__company-logo {
    height: 32px;
    width: auto;
    max-height: 32px;
    max-width: 200px;
...
function renderFooter(
  ...
  const project = PROJECT_DATA[projectKey];
  const company = escapeAttr(JSON.stringify(project.company));
  const links = renderFooterLinks(project.footerLinks);
  const extra = project.extra
    ? `<div class="nds-footer__extra">${escapeHtml(project.extra)}</div>`
    : "";
  const footerLogo = project.footerLogo ?? project.logo;
  /* runmile 처럼 로고 자산 없이 워드마크만 쓰는 project 는 footerLogo.src 가 "" —
  ...
  if (surface === "web") {
    return `
      ${linkReset}
      <nds-footer-web tone="${project.footerTone}" max-width="${project.maxWidth}">
  ...
  const maxWrap =
    layout === "desktop"
      ? `style="max-width:${project.maxWidth}px;margin:0 auto;"`
      : `style="max-width:100%;"`;
  const darkStyle =
    project.footerTone === "dark"
...
/* ──────────────── BottomNav (앱 하단 5탭) ──────────────── */
/* React 의 project BottomNav 와 동등 — 제네릭 <nds-footer-tab-bar>/<nds-footer-tab-item>
...
function renderBottomNav(projectKey: ProjectKey, activeKey: string, _assetBaseUrl: string): string {
  const data = PROJECT_DATA[projectKey].bottomNav;
  // web 전용 project (cashwalk-biz) 는 BottomNav 가 없음 → 빈 렌더.
  if (!data) return "";
...
export class NdsProjectHeader extends NdsElement {
  static elementName = "nds-project-header";
  static projectFallback: ProjectKey = "nudge-eap";

  static get observedAttributes(): readonly string[] {
    return ["project", "surface", "active-key", "asset-base-url"];
  }

  /** 마지막 full-render 의 project|surface|assetBaseUrl 키 — 같으면 in-place 패치만. */
  private _renderedKey: string | null = null;

  protected update(): void {
    const project = normalizeProject(
      this.getAttribute("project"),
      (this.constructor as typeof NdsProjectHeader).projectFallback,
    );
    ...
    this.setAttribute("data-project", project);

    /* active-key 만 바뀐 경우 innerHTML 재빌드 금지 — 검색 input 등 내부 노드가
     * 통째로 재생성돼 포커스가 유실된다 (mount-once 계약,
     * packages/html/test/nds-project-chrome.test.ts 가 잠근다).
     * 메뉴 활성 표시(data-active)만 data-key 앵커에 in-place 패치한다. */
    const renderedKey = `${project}|${surface}|${assetBaseUrl}`;
    ...
    this.innerHTML = renderHeader(project, surface, activeKey, assetBaseUrl);
  }
}

export class NdsProjectFooter extends NdsElement {
  static elementName = "nds-project-footer";
  static projectFallback: ProjectKey = "nudge-eap";

  static get observedAttributes(): readonly string[] {
    return ["project", "surface", "layout", "asset-base-url"];
  }

  protected update(): void {
    const project = normalizeProject(
      this.getAttribute("project"),
      (this.constructor as typeof NdsProjectFooter).projectFallback,
    );
    const projectDefaults = PROJECT_DATA[project];
    const surface = normalizeSurface<FooterSurface>(
      this.getAttribute("surface"),
      ["web", "app"],
      projectDefaults.footerSurface,
    );
    const layout = this.getAttribute("layout") ?? "desktop";
    const assetBaseUrl = this.getAttribute("asset-base-url") ?? "/assets";
    this.setAttribute("data-project", project);
    this.innerHTML = renderFooter(project, surface, layout, assetBaseUrl);
  }
}

export class NdsProjectBottomNav extends NdsElement {
  static elementName = "nds-project-bottom-nav";
  static projectFallback: ProjectKey = "nudge-eap";

  static get observedAttributes(): readonly string[] {
    return ["project", "active-key", "asset-base-url"];
  }

  protected update(): void {
    const project = normalizeProject(
      this.getAttribute("project"),
      (this.constructor as typeof NdsProjectBottomNav).projectFallback,
    );
    const activeKey = this.getAttribute("active-key") ?? "home";
    const assetBaseUrl = this.getAttribute("asset-base-url") ?? "/assets";
    this.setAttribute("data-project", project);
    this.innerHTML = renderBottomNav(project, activeKey, assetBaseUrl);
  }
}

function defineProjectAliases(project: ProjectKey): void {
  class ProjectHeaderAlias extends NdsProjectHeader {
    static override elementName = `nds-${project}-header`;
    static override projectFallback = project;
  }
  class ProjectFooterAlias extends NdsProjectFooter {
    static override elementName = `nds-${project}-footer`;
    static override projectFallback = project;
  }
  /* cashwalk-biz 는 BottomNav 데이터가 없어 빈 렌더 — header/footer 와 대칭을 위해
   * alias 자체는 등록하되 renderBottomNav 가 "" 를 반환한다 (web-only 일관). */
  class ProjectBottomNavAlias extends NdsProjectBottomNav {
    static override elementName = `nds-${project}-bottom-nav`;
    static override projectFallback = project;
  }
  define(ProjectHeaderAlias);
  define(ProjectFooterAlias);
  define(ProjectBottomNavAlias);
}

/* ============================================================================
 * nds-sidebar (흡수) — 옛 standalone packages/html/src/components/nds-sidebar.ts 를
 * 이 파일로 병합(chrome 정리 Phase 3). admin/사이드바도 프로젝트 chrome 의 한 형태.
 * element 명은 호환을 위해 <nds-sidebar> 유지(코드 위치만 nds-project-chrome 로 이전).
...
/**
 * `project` 속성 → 로고 자동 주입 맵. `<nds-project-header>` 와 동일 SSOT(project-logo-defaults).
 *
 ...
 */
interface SidebarProjectLogo {
  src: string;
  alt: string;
  width: number;
  height: number;
}
const SIDEBAR_PROJECT_LOGOS: Record<string, SidebarProjectLogo> = {
  "cashwalk-biz": {
    src: CASHWALK_BIZ_LOGO_DATA_URI,
    alt: "Cashwalk for Business",
    // Storybook(Projects/CashwalkBiz/Sidebar = SSOT) · 시안(Figma 3304:617) 정합:
...
const sidebarStyles = `
  ...
  :where([data-project="cashwalk-biz"] .${SB_HEADER_CLASS}) { justify-content: center; }
  :where([data-project="cashwalk-biz"] .${SB_LOGO_CLASS} img) { max-height: none; height: 56px; width: auto; }
   ...
   *   · active bg = var(--semantic-bg-brand-subtle) → cashwalk = Yellow/100 #FFFAE5 (cashwalk-biz.semantic.ts bg.project.subtle)
   ...
   *   3) CTA / footer 액션 — base 는 solid=primary(노랑) · outlined=project(노랑 보더 + 주황 텍스트).
   ...
   */
  :where([data-project="cashwalk-biz"] .${SB_ROOT_CLASS}) {
    --nds-sidebar-item-active-radius: 16px;
    --nds-sidebar-text-active: ${cv.textRole.normal};
  }
  :where([data-project="cashwalk-biz"] .${SB_ACTION_CLASS}) { border-radius: 8px; }
  :where([data-project="cashwalk-biz"] .${SB_ACTION_CLASS}[data-variant="solid"]) {
    background: ${cv.button.bgSecondary}; color: ${cv.button.textSecondary};
  }
  :where([data-project="cashwalk-biz"] .${SB_ACTION_CLASS}[data-variant="solid"]:hover) { background: ${cv.button.bgSecondaryHover}; }
  :where([data-project="cashwalk-biz"] .${SB_ACCOUNT_ACTIONS_CLASS} .${SB_ACTION_CLASS}[data-variant="outlined"]) {
    color: ${cv.textRole.strong}; border-color: ${cv.textRole.strong};
  }
  :where([data-project="cashwalk-biz"] .${SB_ACCOUNT_ACTIONS_CLASS} .${SB_ACTION_CLASS}[data-variant="outlined"]:hover) { background: ${cv.surface.subtle}; }
  :where([data-project="cashwalk-biz"] .${SB_FOOTER_ACTIONS_CLASS} .${SB_ACTION_CLASS}[data-variant="outlined"]) {
    color: ${cv.textRole.strong}; border-color: ${cv.borderRole.normal}; border-radius: 28px; height: 48px;
  }
  :where([data-project="cashwalk-biz"] .${SB_FOOTER_ACTIONS_CLASS} .${SB_ACTION_CLASS}[data-variant="outlined"]:hover) { background: ${cv.surface.subtle}; }
`;
...
export class NdsSidebar extends NdsElement {
  ...
  protected update(): void {
    ...
    // 로고: 명시 logo-src 가 1순위, 없으면 project 속성으로 project-logo-defaults 에서 자동 주입.
    //   → 35KB base64 를 손으로 붙일 필요가 사라져 "거대 블롭 추출 중 한글 깨짐 + 로고 유실" 회귀를 차단.
    const project = this.getAttribute("project");
    const projectLogo = project ? SIDEBAR_PROJECT_LOGOS[project] : undefined;
    const logoSrc = this.getAttribute("logo-src") ?? projectLogo?.src ?? null;
    const logoAlt = this.getAttribute("logo-alt") ?? projectLogo?.alt ?? "";
    const logoWidth =
      this.getAttribute("logo-width") ?? (projectLogo ? String(projectLogo.width) : null);
    const logoHeight =
      this.getAttribute("logo-height") ?? (projectLogo ? String(projectLogo.height) : null);
    ...
    // project 를 root 에 미러 → [data-project="..."] 스코프 CSS(로고 사이즈 등)가
    // shell 래퍼 없이 standalone <nds-sidebar project="..."> 에서도 적용된다.
    if (project) this._root.dataset.project = project;
...
define(NdsProjectHeader);
define(NdsProjectFooter);
define(NdsProjectBottomNav);
PROJECT_KEYS.forEach(defineProjectAliases);
define(NdsSidebar);
