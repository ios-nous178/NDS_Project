/**
 * nds-address-search DOM 구조 + 검색 / 선택 / 상세 입력 동작 검증.
 */

import { describe, expect, it } from "vitest";
import { NdsAddressSearch } from "../src/components/nds-address-search.js";
...
describe("nds-address-search — DOM parity with React AddressSearch", () => {
  it("registers as a custom element", () => {
    expect(customElements.get("nds-address-search")).toBe(NdsAddressSearch);
  });

  it("renders label + search row with input and 검색 button", async () => {
    const el = document.createElement("nds-address-search");
    el.setAttribute("label", "배송지");
    el.setAttribute("query", "테헤란");
    document.body.appendChild(el);
    await flush();

    const root = el.querySelector(".nds-address-search") as HTMLElement;
    expect(root.dataset.slot).toBe("root");
    expect(root.querySelector(".nds-address-search__label")!.textContent).toBe("배송지");

    const input = root.querySelector("input.nds-address-search__input") as HTMLInputElement;
  ...
  it("typing updates query attribute and emits address-query", async () => {
    ...
    const input = el.querySelector("input.nds-address-search__input") as HTMLInputElement;
    input.value = "강남";
  ...
  it("clicking 검색 button dispatches address-search and shows result block", async () => {
    ...
    const result = el.querySelector(".nds-address-search__result") as HTMLElement;
  ...
  it("results attribute renders list and clicking item emits address-change", async () => {
    ...
    const items = el.querySelectorAll(".nds-address-search__result-item");
  ...
  it("detail input updates value JSON", async () => {
    ...
    const detail = el.querySelector(".nds-address-search__detail") as HTMLElement;
    expect(detail).not.toBeNull();
  ...
  it("error sets input data-error and helper data-error", async () => {
    ...
    const helper = el.querySelector(".nds-address-search__helper") as HTMLElement;
