/**
 * nds-project-bottom-nav — 프로젝트별 5탭 BottomNav 가 PROJECT_DATA 에서 자동 렌더되는지 검사.
 * React 의 project BottomNav (Trost/Geniet/NudgeEAP/Runmile) 와 동등한 탭/아이콘/색 cascade.
 */

import { describe, expect, it } from "vitest";
import { NdsProjectBottomNav } from "../src/components/nds-project-chrome.js";

const flush = () => new Promise<void>((r) => setTimeout(r, 0));

async function mount(project: string, activeKey?: string): Promise<HTMLElement> {
  const el = document.createElement("nds-project-bottom-nav");
...
describe("nds-project-bottom-nav", () => {
  it("registers as a custom element", () => {
    expect(customElements.get("nds-project-bottom-nav")).toBe(NdsProjectBottomNav);
  });

  it("registers per-project aliases", () => {
    for (const tag of [
  ...
  it("renders 5 tabs with project labels (trost)", async () => {
