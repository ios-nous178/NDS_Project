...
import { NdsResultState } from "../src/components/nds-result-state.js";

const flush = () => new Promise<void>((r) => setTimeout(r, 0));

describe("nds-result-state", () => {
  it("registers as a custom element", () => {
    expect(customElements.get("nds-result-state")).toBe(NdsResultState);
  });

  it("renders default icon, title, description, and action slots", async () => {
    const el = document.createElement("nds-result-state");
    ...
    const root = el.querySelector(".nds-result-state__root") as HTMLElement;
    const icon = el.querySelector(".nds-result-state__icon") as HTMLElement;
    const title = el.querySelector(".nds-result-state__title")!;
    const desc = el.querySelector(".nds-result-state__description")!;
    const action = el.querySelector(".nds-result-state__action")!;
  ...
  it("supports min-height CSS variable and hide-icon", async () => {
    ...
    const root = el.querySelector(".nds-result-state__root") as HTMLElement;
    expect(root.style.getPropertyValue("--nds-result-state-min-height")).toBe("320px");
    expect(el.querySelector(".nds-result-state__icon")).toBeNull();
  });
  ...
  it("forwards a11y attributes to root", async () => {
    ...
    const root = el.querySelector(".nds-result-state__root")!;
