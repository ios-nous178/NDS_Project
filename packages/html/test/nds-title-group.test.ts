/**
 * nds-title-group DOM 구조가 React TitleGroup 가 만드는 DOM 과 동일한지 검사.
 */

import { describe, expect, it } from "vitest";
import { NdsTitleGroup } from "../src/components/nds-title-group.js";

const flush = () => new Promise<void>((r) => setTimeout(r, 0));

describe("nds-title-group — DOM parity with React TitleGroup", () => {
  it("registers as a custom element", () => {
    expect(customElements.get("nds-title-group")).toBe(NdsTitleGroup);
  });

  it("renders heading and subtitle with level mapping (h2 default)", async () => {
    const el = document.createElement("nds-title-group");
    el.setAttribute("title", "안녕");
    el.setAttribute("subtitle", "반가워");
    document.body.appendChild(el);
    await flush();

    const root = el.querySelector(`.nds-title-group`) as HTMLElement;
    const title = el.querySelector(`.nds-title-group__title`) as HTMLElement;
    const subtitle = el.querySelector(`.nds-title-group__subtitle`) as HTMLElement;
  ...
  it("switches heading tag and font when level changes", async () => {
    ...
    let title = el.querySelector(".nds-title-group__title") as HTMLElement;
    ...
    await flush();
    title = el.querySelector(".nds-title-group__title") as HTMLElement;
  ...
  it("omits subtitle when not provided", async () => {
    ...
    document.body.appendChild(el);
    await flush();
    expect(el.querySelector(".nds-title-group__subtitle")).toBeNull();
  ...
  it("forwards aria-label to root", async () => {
    ...
    const root = el.querySelector(".nds-title-group")!;
