/**
 * Project header 출력 구조 잠금.
 *
 * 이 파일의 단일 목적: `<nds-project-header project="...">` 출력이 각 프로젝트 데스크톱 헤더의
 * 시각 요소(로고/메뉴/검색/CTA 등)를 항목별로 빠짐없이 들고 있는지 단단히 잠그는 것.
 *
 * 과거엔 공개 react 프로젝트 chrome(GenietAppBar/TrostWebHeader 등)과의 1:1 정합 테스트였으나,
 * chrome 통합으로 react 프로젝트 chrome 은 제거됐고 이제 `nds-project-chrome` 의 PROJECT_DATA 가
 * 프로젝트 chrome 디자인 SSOT 다. nds-project-chrome.test.ts 가 "기본 동작"이면 여기는 "프로젝트별
 * 완성형 요소" — PROJECT_DATA 가 의도한 요소를 빠뜨리면 여기 한 군데가 빨갛게 떠야 한다.
 *
...
describe("Geniet desktop ↔ React GenietAppBar variant='desktop'", () => {
  it("renders the 2-tier shell (search header + menu header)", async () => {
    const el = await mount({ project: "geniet", "active-key": "home" });
  ...
  it("search row: logo + search input(placeholder) + trending top + 3 action buttons + 1 divider", async () => {
    ...
    const logo = el.querySelector('.nds-project-geniet__logo img[alt="Geniet"]') as HTMLImageElement;
    ...
    const input = el.querySelector(".nds-project-geniet__search-input input") as HTMLInputElement;
    expect(input?.placeholder).toBe("궁금한 음식 칼로리, 다이어트 후기 등을 검색해 보세요");
    /* trending top keyword */
    const trendingKeyword = el.querySelector(".nds-project-geniet__trending-keyword");
    expect(trendingKeyword?.textContent).toBe("고단백 식단");
    expect(el.querySelector(".nds-project-geniet__trending-stamp")?.textContent).toBe("09:00 기준");
    /* login_area — 3 action buttons */
    const actions = el.querySelectorAll(".nds-project-geniet__action-btn");
  ...
  it("menu row: category box + 5-item GNB (active='home') + CTA pills (outline + tinted)", async () => {
    const el = await mount({ project: "geniet", "active-key": "home" });
    /* 음식 카테고리 박스 */
    const category = el.querySelector(".nds-project-geniet__category");
    expect(category?.textContent).toContain("음식 카테고리");
    /* GNB 5탭 */
    const gnb = el.querySelectorAll(".nds-project-geniet__gnb-item");
    ...
    /* 활성 GNB 강조 */
    const active = el.querySelector('.nds-project-geniet__gnb-item[data-active="true"]');
    expect(active?.textContent?.trim()).toBe("홈");
    /* CTA pills */
    const ctas = el.querySelectorAll(".nds-project-geniet__cta-pill");
...
describe("Geniet mobile ↔ React GenietAppBar variant='mobile'", () => {
  it("renders 2-tier mobile shell (row1 + row2) with mobile-specific logo asset", async () => {
    const el = await mount({ project: "geniet", surface: "mobile" });
    expect(el.querySelector(".nds-project-geniet--mobile")).toBeTruthy();
    ...
    const logo = el.querySelector(".nds-project-geniet__mo-row1 img") as HTMLImageElement;
    expect(logo?.getAttribute("src")).toMatch(/^data:image\/webp;base64,/);
  ...
  it("row1: point chip (34,300 + gpoint icon) + user button", async () => {
    const el = await mount({ project: "geniet", surface: "mobile" });
    const chip = el.querySelector(".nds-project-geniet__point-chip") as HTMLAnchorElement;
    expect(chip?.textContent).toContain("34,300");
    expect(chip?.querySelector("svg")).toBeTruthy(); // gpoint icon inline
    expect(el.querySelector('.nds-project-geniet__mo-user[aria-label="사용자"]')).toBeTruthy();
  });

  it("row2: hamburger + search input (음식명/칼로리 placeholder)", async () => {
    const el = await mount({ project: "geniet", surface: "mobile" });
    expect(
      el.querySelector('.nds-project-geniet__mo-hamburger[aria-label="음식 카테고리"]'),
    ).toBeTruthy();
    const input = el.querySelector(".nds-project-geniet__mo-search input") as HTMLInputElement;
 ...
 *   목업 셸: nds-project-chrome renderTrostHeader (3단 desktop 헤더)
 *     · 띠 배너 (50h, project.banner → renderProjectBanner, "기업 전용 멘탈케어" + 넛지EAP CTA)
...
describe("Trost web ↔ React TrostWebHeader (compound)", () => {
  it("EAP banner: strong + body text + CTA accent '넛지EAP'", async () => {
    const el = await mount({ project: "trost" });
    const banner = el.querySelector(".nds-project-banner");
    expect(banner).toBeTruthy();
    expect(banner?.getAttribute("href")).toBe("https://eapkorea.co.kr/");
    const bannerText = el.querySelector(".nds-project-banner__text");
    ...
    const ctaAccent = el.querySelector(".nds-project-banner__cta-label span");
  ...
  it("utility header: logo + yellow-border search input (530w) + login(+상담사회원가입) + app-dl", async () => {
    ...
    const logo = el.querySelector(
      '.nds-project-trost-web__utility-logo img[alt="Trost"]',
    ) as HTMLImageElement;
    expect(logo?.getAttribute("src")).toMatch(/^data:image\/svg\+xml;base64,/);
    /* search input */
    const search = el.querySelector(".nds-project-trost-web__search input") as HTMLInputElement;
    ...
    const styleTag = document.querySelector(
      'style[data-nds-style="nds-project-chrome-trost-web"]',
    ) as HTMLStyleElement | null;
    expect(styleTag?.textContent).toContain("width: 530px");
    /* login 2 buttons (로그인 + 상담사 회원가입) */
    const loginBtns = el.querySelectorAll(".nds-project-trost-web__login-btn");
    expect(Array.from(loginBtns).map((b) => b.textContent?.trim())).toEqual([
      "로그인",
      "상담사 회원가입",
    ]);
    /* app download button */
    expect(el.querySelector(".nds-project-trost-web__app-dl")?.textContent?.trim()).toBe(
      "앱 다운로드",
    );
  });
  ...
  it("tab navigation: 6 tabs, 'N' badge on 커뮤니티, active='/' → 홈", async () => {
    const el = await mount({ project: "trost" });
    const tabs = el.querySelectorAll(".nds-project-trost-web__tabnav-link");
    ...
    /* "커뮤니티" 만 isNew badge */
    const newBadges = el.querySelectorAll(".nds-project-trost-web__tabnav-new");
    expect(newBadges.length).toBe(1);
    expect(newBadges[0].closest(".nds-project-trost-web__tabnav-link")?.textContent).toContain(
      "커뮤니티",
    );
    /* active = "/" */
    const active = el.querySelector('.nds-project-trost-web__tabnav-link[data-active="true"]');
    expect(active?.textContent?.trim()).toBe("홈");
  });
});
 ...
 *     · 1-tier 80h, 메뉴 absolute 중앙정렬, appDownload(F5F5F5) + auth(project border)
...
describe("NudgeEAP web ↔ React NudgeEAPWebHeader", () => {
  it("renders 1-tier shell with 6 menu items + absolute-centered nav", async () => {
    const el = await mount({ project: "nudge-eap", "active-key": "counsel" });
    expect(el.querySelector(".nds-project-nudge-eap-web")).toBeTruthy();
    const menu = el.querySelectorAll(".nds-project-nudge-eap-web__menu-item");
    ...
    /* active strong + project color */
    const active = el.querySelector('.nds-project-nudge-eap-web__menu-item[data-active="true"]');
    expect(active?.textContent?.trim()).toBe("상담하기");
    /* 메뉴 absolute centering 은 css rule 로 보장 */
    const styleTag = document.querySelector(
      'style[data-nds-style="nds-project-chrome-nudge-eap-web"]',
    ) as HTMLStyleElement | null;
  ...
  it("right actions: app download (앱 다운로드) + login auth pill", async () => {
    const el = await mount({ project: "nudge-eap" });
    const appDl = el.querySelector(".nds-project-nudge-eap-web__app-dl") as HTMLAnchorElement;
    expect(appDl?.textContent).toBe("앱 다운로드");
    expect(appDl?.getAttribute("href")).toBe("/download");
    expect(el.querySelector(".nds-project-nudge-eap-web__auth")?.textContent).toBe("로그인");
  });
});
...
describe("CashwalkBiz web ↔ React CashwalkBizWebHeader variant='desktop'", () => {
  it("renders 5-tab menu + login + yellow primary CTA pill (#FFD200)", async () => {
    const el = await mount({ project: "cashwalk-biz", "active-key": "ad" });
    const menu = el.querySelectorAll(".nds-project-cashwalk-biz__menu-item");
    expect(menu.length).toBe(5);
    // Figma 98:1082 마케팅 GNB 5탭 (Storybook SSOT). nds-project-chrome.ts webMenu 와 일치.
    expect(Array.from(menu).map((m) => m.textContent?.trim())).toEqual([
      "채널",
      "광고",
      "성공사례",
    ...
    const cta = el.querySelector(".nds-project-cashwalk-biz__primary-cta") as HTMLAnchorElement;
    expect(cta?.textContent).toBe("광고 시작하기");
    expect(cta?.getAttribute("href")).toBe("/start");
    /* yellow #FFD200 is the project signature — css rule 에 박혀 있어야 함 */
    const styleTag = document.querySelector(
      'style[data-nds-style="nds-project-chrome-cashwalk-biz"]',
    ) as HTMLStyleElement | null;
...
describe("CashwalkBiz mobile ↔ React CashwalkBizWebHeader variant='mobile'", () => {
  it("renders compact bar: logo (74×24) + hamburger", async () => {
    const el = await mount({ project: "cashwalk-biz", surface: "mobile" });
    expect(el.querySelector(".nds-project-cashwalk-biz__inner--mobile")).toBeTruthy();
    const logo = el.querySelector('img[alt="Cashwalk for Business"]') as HTMLImageElement;
    expect(logo?.getAttribute("width")).toBe("74");
    expect(el.querySelector('.nds-project-cashwalk-biz__hamburger[aria-label="메뉴"]')).toBeTruthy();
    /* 메뉴/CTA 는 모바일에서 미노출 */
    expect(el.querySelector(".nds-project-cashwalk-biz__menu")).toBeNull();
    expect(el.querySelector(".nds-project-cashwalk-biz__primary-cta")).toBeNull();
  });
});
