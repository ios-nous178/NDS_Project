...
const SRC_DIR = "/tmp/figma-cashpobi";
...
const FROM_FIGMA = [
  ...
  "arrow-down",
  "arrow-right",
  "calendar", // DatePicker 트리거에서 brand 별 swap — Cashpobi 는 filled + dot grid 변종.
  "caution",
  "check",
  "check-circle-on",
  "check-circle-off",
...
function main() {
  ...
  // 1) Figma 다운 받은 SVG 들 정제 후 cashpobi-* 로 출력.
  for (const name of FROM_FIGMA) {
    ...
    const outPath = join(SVG_DIR, `cashpobi-${name}.svg`);
    writeFileSync(outPath, out);
    console.log(`  ✓ ${name} → cashpobi-${name}.svg`);
    written++;
  }

  // 2) 인라인 SVG (합성/CSS-only) 들 cashpobi-* 로 출력.
  for (const [name, svg] of Object.entries(INLINE)) {
    const outPath = join(SVG_DIR, `cashpobi-${name}.svg`);
    writeFileSync(outPath, svg);
    console.log(`  ✓ inline → cashpobi-${name}.svg`);
    written++;
  }

  console.log(`\n  Wrote ${written} Cashpobi icons to ${SVG_DIR}`);
}
