import React from "react";

export interface GenietHomeOnIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
}

export const GenietHomeOnIcon = React.forwardRef<SVGSVGElement, GenietHomeOnIconProps>(
  ({ size = 24, color = "currentColor", ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      color={color}
      {...props}
    >
      <g transform="scale(1.274021 1.24998)">
<path id="Fill-1" fillRule="evenodd" clipRule="evenodd" d="M11.658 16.9552H7.18C6.719 16.9552 6.346 16.5822 6.346 16.1222C6.346 15.6612 6.719 15.2882 7.18 15.2882H11.658C12.119 15.2882 12.492 15.6612 12.492 16.1222C12.492 16.5822 12.119 16.9552 11.658 16.9552ZM17.743 6.10625L10.569 0.40125C9.896 -0.13375 8.943 -0.13375 8.27 0.40125L1.095 6.10625C0.403 6.65625 0 7.49225 0 8.37625V16.5182C0 17.9992 1.201 19.2003 2.682 19.2003H16.156C17.637 19.2003 18.838 17.9993 18.838 16.5173V8.37625C18.838 7.49225 18.435 6.65625 17.743 6.10625Z" fill="currentColor"/>
</g>
    </svg>
  )
);

GenietHomeOnIcon.displayName = "GenietHomeOnIcon";
