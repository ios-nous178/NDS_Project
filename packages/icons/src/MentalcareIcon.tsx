import React from "react";

export interface MentalcareIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
}

export const MentalcareIcon = React.forwardRef<SVGSVGElement, MentalcareIconProps>(
  ({ size = 24, color = "currentColor", ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      color={color}
      {...props}
    >
      <g id="Group 4739">
        <g id="Rectangle 2537"></g>
        <path
          id="Vector"
          d="M2 9.50001C2.00002 8.38721 2.33759 7.30059 2.96813 6.38367C3.59867 5.46675 4.49252 4.76267 5.53161 4.36441C6.5707 3.96615 7.70616 3.89245 8.78801 4.15305C9.86987 4.41365 10.8472 4.99629 11.591 5.82401C11.6434 5.88002 11.7067 5.92468 11.7771 5.95521C11.8474 5.98574 11.9233 6.00149 12 6.00149C12.0767 6.00149 12.1526 5.98574 12.2229 5.95521C12.2933 5.92468 12.3566 5.88002 12.409 5.82401C13.1504 4.99091 14.128 4.40338 15.2116 4.13961C16.2952 3.87585 17.4335 3.94836 18.4749 4.34749C19.5163 4.74663 20.4114 5.45346 21.0411 6.37391C21.6708 7.29436 22.0053 8.38477 22 9.50001C22 11.79 20.7 14 19.2 15.5C17.7 17 13.7062 20.1463 13.508 20.313C13.3098 20.4797 13.0919 20.6989 12.834 20.8173C12.5762 20.9357 12.296 20.9979 12.0123 20.9997C11.7285 21.0015 11.4476 20.9428 11.1883 20.8277C10.9289 20.7126 10.7526 20.5247 10.508 20.332C10.2634 20.1393 6.3 17 4.8 15.5C3.3 14 2 11.8 2 9.50001Z"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          id="Vector_2"
          d="M18 11.7795H17.199C16.369 11.7795 15.954 11.7795 15.61 11.9745C15.266 12.1685 15.053 12.5245 14.626 13.2355L14.596 13.2875C14.198 13.9505 13.999 14.2815 13.71 14.2765C13.421 14.2715 13.234 13.9325 12.861 13.2545L11.174 10.1875C10.827 9.55548 10.654 9.23948 10.376 9.22448C10.099 9.20948 9.892 9.50448 9.479 10.0945L9.196 10.4995C8.756 11.1265 8.537 11.4395 8.212 11.6095C7.886 11.7795 7.503 11.7795 6.738 11.7795H6"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
        />
      </g>
    </svg>
  ),
);

MentalcareIcon.displayName = "MentalcareIcon";
