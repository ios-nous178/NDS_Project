import React from "react";

export interface MockupBoldMusicCircleIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
}

export const MockupBoldMusicCircleIcon = React.forwardRef<SVGSVGElement, MockupBoldMusicCircleIconProps>(
  ({ size = 24, color = "currentColor", ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      color={color}
      {...props}
    >
      <path d="M2.582 9.42c-.08 0-.17-.01-.25-.04a.763.763 0 0 1-.46-.96c.67-1.88 1.83-3.53 3.38-4.78.32-.26.79-.21 1.05.11s.21.79-.11 1.06a9.244 9.244 0 0 0-2.9 4.11c-.11.31-.4.5-.71.5ZM2.58 16.08a.75.75 0 0 1-.71-.5c-.41-1.16-.62-2.37-.62-3.58 0-.41.34-.75.75-.75s.75.34.75.75c0 1.04.18 2.08.54 3.08.14.39-.07.82-.46.96-.08.03-.17.04-.25.04ZM11.999 22.752c-1.06 0-2.11-.16-3.13-.47a.75.75 0 0 1-.5-.94c.12-.4.54-.62.94-.5.87.27 1.78.4 2.69.4 5.1 0 9.25-4.15 9.25-9.25 0-.52-.05-1.06-.15-1.63-.07-.41.2-.8.61-.87.4-.07.8.2.87.61.12.66.18 1.28.18 1.9-.01 5.93-4.83 10.75-10.76 10.75ZM5.718 20.498a.7.7 0 0 1-.47-.17c-.57-.46-1.03-.9-1.42-1.35a.757.757 0 0 1 .08-1.06c.32-.27.79-.23 1.06.08.33.38.73.76 1.22 1.16.32.26.37.73.11 1.05a.72.72 0 0 1-.58.29ZM20.238 7.09a.74.74 0 0 1-.62-.33 9.247 9.247 0 0 0-10.31-3.61.76.76 0 0 1-.94-.5c-.13-.4.1-.82.5-.94 1.02-.3 2.07-.46 3.13-.46 3.54 0 6.85 1.75 8.86 4.67.23.34.15.81-.19 1.04-.13.09-.28.13-.43.13Z" fill="currentColor"></path><path d="M16.03 6.499c-.33-.25-.93-.5-1.89-.24l-3.19.86c-.92.26-1.52 1.04-1.52 2v4.22c-.26-.1-.54-.16-.84-.16-1.29 0-2.34 1.05-2.34 2.34 0 1.29 1.05 2.34 2.34 2.34 1.28 0 2.31-1.03 2.33-2.3 0-.01.01-.02.01-.04v-4.19l4.32-1.18v2.13c-.26-.1-.54-.16-.84-.16-1.29 0-2.34 1.05-2.34 2.34 0 1.29 1.05 2.34 2.34 2.34 1.29 0 2.34-1.05 2.34-2.34v-6.21c0-.8-.24-1.39-.72-1.75Zm-7.44 9.86c-.46 0-.84-.38-.84-.84 0-.46.38-.84.84-.84.46 0 .84.38.84.84 0 .46-.38.84-.84.84Zm5.82-1.06c-.46 0-.84-.38-.84-.84 0-.46.38-.84.84-.84.46 0 .84.38.84.84 0 .46-.38.84-.84.84Z" fill="currentColor"></path>
    </svg>
  )
);

MockupBoldMusicCircleIcon.displayName = "MockupBoldMusicCircleIcon";
