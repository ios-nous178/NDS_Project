import React from "react";

export interface MockupBoldTruckFastIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
}

export const MockupBoldTruckFastIcon = React.forwardRef<SVGSVGElement, MockupBoldTruckFastIconProps>(
  ({ size = 24, color = "currentColor", ...props }, ref) => (
    <svg
      ref={ref}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      color={color}
      {...props}
    >
      <path d="M21.5 15.5c.28 0 .5.22.5.5v1c0 1.66-1.34 3-3 3 0-1.65-1.35-3-3-3s-3 1.35-3 3h-2c0-1.65-1.35-3-3-3s-3 1.35-3 3c-1.66 0-3-1.34-3-3v-2c0-.55.45-1 1-1h9.5a2.5 2.5 0 0 0 2.5-2.5V6c0-.55.45-1 1-1h.84c.72 0 1.38.39 1.74 1.01l.64 1.12c.09.16-.03.37-.22.37a2.5 2.5 0 0 0-2.5 2.5v3a2.5 2.5 0 0 0 2.5 2.5h2.5Z" fill="currentColor"></path><path d="M8 22a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM16 22a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM22 12.53V14h-3c-.55 0-1-.45-1-1v-3c0-.55.45-1 1-1h1.29l1.45 2.54c.17.3.26.64.26.99ZM13.08 2H5.69a3.69 3.69 0 0 0-3.62 2.98h4.37c.38 0 .68.31.68.69 0 .38-.3.68-.68.68H2v1.38h2.6c.38 0 .69.31.69.69 0 .38-.31.68-.69.68H2v1.38h.77c.38 0 .69.31.69.69 0 .38-.31.68-.69.68H2v.23c0 .55.45 1 1 1h9.15c1.02 0 1.85-.83 1.85-1.85V2.92c0-.51-.41-.92-.92-.92ZM2.07 4.98H.94c-.38 0-.69.31-.69.69 0 .38.31.68.69.68H2v-.66c0-.24.03-.48.07-.71ZM1.85 7.73H.94c-.38 0-.69.31-.69.69 0 .38.31.68.69.68H2V7.73h-.15ZM1.85 10.48H.94c-.38 0-.69.31-.69.69 0 .38.31.68.69.68H2v-1.37h-.15Z" fill="currentColor"></path>
    </svg>
  )
);

MockupBoldTruckFastIcon.displayName = "MockupBoldTruckFastIcon";
