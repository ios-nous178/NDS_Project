...
**프로젝트 앱 하단 BottomNav — 손수 조립하지 말 것.** `<nds-project-bottom-nav project='trost|geniet|nudge-eap|runmile' active-key='home'>` 한 줄로 프로젝트별 5탭 (라벨/아이콘 active·inactive/색)이 PROJECT_DATA 에서 자동 렌더. 제네릭 nds-footer-tab-bar + nds-footer-tab-item 에 아이콘 SVG 를 슬롯으로 직접 주입하는 건 안티패턴.
...
```html
<!-- 손수 조립 안티패턴 — 탭/아이콘/색을 인라인으로 박으면 프로젝트 데이터와 분리되어 다음 화면에서 또 적게 됨 -->
