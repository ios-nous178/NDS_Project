...
단일 선택 버튼 한 개 (브랜드색 아웃라인 + selected 시 project-subtle 채움). 보통 `SelectionButtonGroup` 으로 묶지만, 토글 1개·커스텀 레이아웃이면 단독으로 쓴다. 그룹과 동일한 `nds-selection-button-group__item` 비주얼 SSOT 를 공유 — 5개 프로젝트 시멘틱 cascade 자동 대응.
