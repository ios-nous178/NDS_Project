...
SMS/이메일 인증코드 입력 — 웹용 단일 필드(한 줄 박스). 코드 **입력 필드만** 책임진다. 자리별 세그먼트(네이티브식)가 아니라 base Input 과 동일한 단일 박스라 붙여넣기·자동완성(one-time-code)이 자연 지원되고 높이/둥근모서리는 Input 토큰(--nds-input-*)을 상속한다. 타이머·재전송·확인 버튼이 함께 있는 인증 폼은 이 필드를 **FormField + InputGroup** 으로 합성한다(남은시간 타이머는 앱이 합성하는 인라인 요소로 코드 입력 우측에 겹쳐 배치, 확인 버튼은 InputGroup 의 같은 행). (구 이름 OtpInput — 2026-06 VerificationCodeInput 으로 개명, 태그 nds-verification-code-input.)
...
```html
<!-- 레시피A · 인라인 확인 버튼형: 코드 입력 + 확인 버튼을 FormField > InputGroup 으로, 타이머는 코드 입력 우측에 겹쳐 배치.
