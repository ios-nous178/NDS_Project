...
캐시워크 포 비즈니스 admin **광고비/충전 안내 배너** — 페이지 본문 상단(페이지 헤더 아래)에 얹는 soft 옐로우 알림 카드. 구성: 좌측 일러스트(@nudge-design/assets `charge-alert-bell`) + 제목/설명 + 우측 단일 CTA(노란 pill). NoticeAlert(48px 인라인 strip)·Banner(우측 이미지)·FloatingCtaBanner(하단 sticky pill)와 구분 — 일러스트 동반 멀티라인 안내 카드. Figma 3001-21374.
