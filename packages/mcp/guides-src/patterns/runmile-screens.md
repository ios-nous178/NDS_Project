...
Runmile(러닝 대회·레이스 모집/참가 + 러너 커뮤니티) 화면의 **시각 레퍼런스 카탈로그** — 웹 홈(대회 모집) / 홈+채팅 드로어 / 대회 상세 3종 스크린샷을 담는다. Runmile 은 대회 포스터 일러스트가 시각 주인공이라 UI 채도는 낮추고 코랄·핑크 브랜드 컬러를 신청 CTA·상태 배지·링크에만 쓴다. 화면을 새로 짤 때 **레이아웃·구성·위계의 출처**로 이 스크린샷들을 먼저 보고(`references[].imageAbsolutePath`), 정확한 색·여백·치수·컴포넌트 선택은 해당 `component:*` 가이드와 브랜드 토큰을 SSOT 로 따른다.
