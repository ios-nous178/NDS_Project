...
export const COMPONENT_GUIDES: Record<string, ComponentGuide> = {
  ...
  "Article": {
    "name": "Article",
    "summary": "게시글/공지/FAQ **상세 보기** 셸. 제목·메타(Header) + 본문(Body=ContentViewer) + 첨부(Attachments) + 액션(Actions)을 묶는 레이아웃 compound. 좋아요 카운트·권한·작성자 데이터 등 **앱 로직은 0** — 모두 슬롯에 주입한다.",
    "pitfalls": [
      "**목록/피드의 게시글 \"카드\"는 Article 이 아니다** — Card(Header/Title/Subtitle/Meta/Thumbnail) + ListItem 조합 패턴. Article 은 **상세 한 건** 전용.",
      "본문은 `Article.Body` 의 `html` prop → 내부에서 ContentViewer 로 sanitize 렌더(태그 allowlist). raw `<div dangerouslySetInnerHTML>` 직접 쓰지 말 것. 신뢰된 본문이면 `sanitize={false}`.",
      ...
      "html 은 container + subpart 엘리먼트(`<nds-article>`/`<nds-article-header|body|attachments|actions>`)로 미러 — 본문엔 `<nds-content-viewer>` 를 넣는다."
    ],
    "recommended": [
