...
export const COMPONENT_GUIDES: Record<string, ComponentGuide> = {
  ...
  "Button": {
    ...
    "colorMatrix": {
      ...
      "neutral/outlined-subtle": "흰 배경 + 옅은 보더(borderRole.subtle) + neutral 텍스트 — 캐시워크 가이드(262:1815)의 주력 저강조 보조 액션(가장 낮은 강조)",
      "danger/outlined-subtle": "흰 배경 + 옅은 보더 + var(--semantic-text-status-error) red 텍스트 — 저강조 위험 액션(삭제 보조 등). 캐시워크 가이드 인가 조합",
      "danger/solid": "var(--semantic-fill-status-error) red fill + 흰 텍스트 — 파괴 액션 한정 (구 error/solid)"
    },
    "sizeMatrix": {
      ...
      "sm": "height 40 / px 16 / py 11 / 14·20 bold / icon 20 / gap 8",
      "xs": "height 36 / px 16 / py 10 / 13·18 bold / icon 18 / gap 6"
    },
    "stateMatrix": {
    ...
    "matrixOverrides": {
      "runmile": {
        "sizeMatrix": {
          ...
          "sm": "height 44 (base 40 → 44) / px 16 / 14·20 bold / icon 20 / gap 8",
      ...
      "cashwalk-biz": {
        "sizeMatrix": {
          "sm": "height 40 — base 와 동일(캐시워크 ButtonGuide 262:1815 공통, 더 이상 override 아님)",
          "xs": "height 36 — base 와 동일(캐시워크 ButtonGuide 262:1815 공통, 더 이상 override 아님)"
        },
        "stateMatrix": {
...
export const PATTERN_GUIDES: Record<string, PatternGuide> = {
  ...
  "card-grid": {
    ...
    "summary": "홈·마이페이지·갤러리·추천처럼 **콘텐츠 카드를 다열로 배치**하는 그리드는 raw `display:grid` 를 매번 손코딩하지 말고 레이아웃 프리미티브 **`.nds-grid` 클래스**로 만든다. `<div class=\"nds-grid\" data-cols=\"3\">` 안에 `<nds-card>`(또는 Card) 셀을 넣으면 토큰 간격·반응형 1열 fallback·min-width:0 깨짐방지가 한 번에 적용된다. 열 수·간격은 전부 data 속성/토큰으로 제어 — 컴포넌트(`<nds-grid>`)가 아니라 클래스다(Layout primitive 컨벤션, shell·container·section 과 동일). Card 본문 룰은 get_guide({ topic: 'component:Card' }) 와 함께 본다.",
    "rules": [
