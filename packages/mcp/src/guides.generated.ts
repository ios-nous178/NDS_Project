...
export const COMPONENT_GUIDES: Record<string, ComponentGuide> = {
  ...
  "BottomNav": {
    ...
    "summary": "모바일 앱 하단 글로벌 네비게이션(3~5탭) 전용 primitive. compound + 슬롯 — `<BottomNav activeKey onChange>` 안에 `<BottomNav.Item itemKey label icon activeIcon href badge>`. 브랜드를 모르는 컴포넌트로, 색은 `--nds-bottomnav-*` 슬롯으로 노출되고 브랜드 토큰이 값만 덮는다. 브랜드별 아이콘/라벨은 호출부가 주입한다(`{Brand}BottomNav` 래퍼 대체).",
    "pitfalls": [
      ...
      "색을 컴포넌트에 박지 말 것 — 브랜드 활성색은 `--nds-bottomnav-active-color`, 비활성은 `--nds-bottomnav-inactive-color` 슬롯으로 브랜드 토큰 파일이 덮는다.",
  ...
  "BottomSheet": {
    ...
    "summary": "모바일에서 화면 하단에서 올라오는 시트. 옵션 선택 / 짧은 작업에 적합. 데스크탑에선 Drawer 가 자연스러움. 공유 시트는 이 컴포넌트로 4칸 그리드 + 링크 복사 레시피로 조립한다. 시각 토큰은 --nds-bottom-sheet-* 슬롯(radius·handle-width·shadow·backdrop)으로 노출돼 브랜드 토큰이 값만 덮는다 — Trost 는 radius 20·handle 40 을 emit(컴포넌트는 브랜드를 모름, base 는 12/36 불변). Share/Info/List 는 별도 prop·variant 가 아니라 BottomSheet 본체 + DS 토큰으로 조립하는 컴포지션 레시피다.",
    "pitfalls": [
  ...
  "BrandBottomNav": {
    ...
    "summary": "**브랜드 앱 하단 BottomNav — 손수 조립하지 말 것.** `<nds-brand-bottom-nav brand='trost|geniet|nudge-eap|runmile' active-key='home'>` 한 줄로 브랜드별 5탭 (라벨/아이콘 active·inactive/색)이 BRAND_DATA 에서 자동 렌더. 제네릭 nds-footer-tab-bar + nds-footer-tab-item 에 아이콘 SVG 를 슬롯으로 직접 주입하는 건 안티패턴.",
    "pitfalls": [
  ...
  "BrandFooter": {
    ...
    "summary": "**브랜드 글로벌 푸터 — 손수 조립하지 말 것.** `<nds-brand-footer brand='...' surface='web|app'>` 한 줄로 이용약관/개인정보처리방침/사업자정보/copyright/푸터 로고가 BRAND_DATA 에서 자동 렌더. nds-footer + nds-footer-links + nds-footer-company 직접 조립 = 안티패턴.",
    "pitfalls": [
  ...
  "FloatingCtaBanner": {
    ...
    "summary": "페이지 하단 sticky CTA 배너. pill (radius 100) + brand border 1px + shadow. 좌측 일러스트(leadingIcon) + 캡션(보조) + 강조 CTA 텍스트 + 우측 chevron 아이콘. 기본 `floating=true` 시 position:fixed 로 화면 하단 중앙 자동 고정.",
    "pitfalls": [
  ...
  "Header": {
    ...
    "summary": "base 헤더. variant 로 분기: compact(모바일 56px flex) / webview(56px, title 중앙 + back) / transparent(56px, 배경 투명) / web(데스크탑 80px grid 3열, max-width 1200). 브랜드 화면이면 base Header 가 아니라 brand chrome (TrostAppBar / NudgeEAPWebHeader / CashwalkBizWebHeader 등) 사용.",
    "pitfalls": [
  ...
  "List": {
    ...
    "recommended": [
      ...
      "(Trost) PC Table(기본 표): <List platform='pc'><ListItem layout='table' onSelect={…}><span data-col='date'>2026.05.20</span><span data-col='category'>개인 상담</span><span data-col='name'>김민지 상담사</span><span data-col='status'>완료</span></ListItem></List> — body 가 가로 행, name 이 flex-spacer, status 는 brand 색. 풍부한 표는 DataTable.",
      "(Trost) PC Compact: <List platform='pc'><ListItem layout='compact' title='푸시 알림' trailing={<Toggle/>} /></List> — h42 고밀도, PC only.",
      "(Trost) Mobile Thumbnail + 액션 링크: <List platform='mobile'><ListItem layout='thumbnail' leading={<썸네일 72/>} title='메뉴' description='설명' metadata='320 kcal · 4.5 ★' actionLink='주문 다시하기' onActionLinkSelect={…} /></List> — h124, metadata 아래 brand 색 인라인 링크(metadata 와 구분). onActionLinkSelect 는 Row onSelect 와 분리(stopPropagation).",
  ...
  "Modal": {
    ...
    "pitfalls": [
      ...
      "**런마일(Modal 가이드 5085:27)**: 컨테이너 radius **3XL 24** · **Elevation/3**(0 4px 13px α0.06) · **Title=Text/Strong**(#221E1F) · **Body=Text/Normal**(subtle #4E5968) **13/18**. 모두 `components.modal`(radius/shadow/titleColor/bodyColor/bodyFontSize/bodyLineHeight) 슬롯 — 타 브랜드 fallback 유지. overlay #000 α0.5 · 흰 surface · Confirm=오렌지 Solid · Cancel=Outlined 좌 + Confirm 우(actionsLayout split). 단일=Confirm only, 이중=Cancel+Confirm. 본문 3줄↑/입력 폼이면 BottomSheet·별도 페이지 권장. (치수 296/328 은 React DEVICE_WIDTH 하드코딩이라 base 294/332 유지 — 4px 차.) **Popup**(가운데 confirm)도 런마일 radius 20·Elevation/3 정합(`components.popup`)."
    ],
    "examplesHtml": {
  ...
  "PhoneInput": {
    ...
    "summary": "국가 코드 + 휴대폰 번호 입력. ISO code 관리 + 다이얼 코드는 countries 데이터에서 조회. 국기 이모지 없이 다이얼코드/ISO코드/국가명만 표시(텍스트 이모지 금지 규칙 준수). 레이아웃 = 국가코드 드롭다운 박스 + 번호 입력 박스가 분리된 두 박스(gap). 두 박스 모두 base Input 시멘틱 토큰(height 48 · radius md · border/background)을 상속하므로 brand cascade(--nds-input-*)가 그대로 적용됨.",
    "pitfalls": [
  ...
  "QuickMenu": {
    ...
    "summary": "PC 화면 우측 고정(sticky/fixed) 영역에서 자주 쓰는 **전역 액션 2~4개(3개 권장)**를 빠르게 노출하는 보조 navigation. Container(width 120 · radius 12 · White · overlay shadow) + Header(\"QUICK MENU\" Bold 13 brand) + Menu Item × N(IconCircle 60 + 라벨) + 하단 TOP(맨 위로) 버튼으로 구성. 고빈도·즉시성·전역 도달이 핵심 — 페이지별 컨텍스트 액션과 분리한다.",
    "pitfalls": [
  ...
  "Sidebar": {
    ...
    "pitfalls": [
      "**★ 어드민(b2b 하드게이트 브랜드) = nds-sidebar 가 정답.** 사내 백오피스는 antd Layout.Sider 가 규칙이지만(get_guide({ topic:'backoffice' })), 어드민(외부 제공 b2b)은 캐포비(cashwalk-biz)·넛지EAP(nudge-eap)만 지원하며 antd 가 아니라 이 목업 전용 `<nds-sidebar>`(nds-brand-chrome 흡수) 를 쓴다. (공개 react `Sidebar` 컴포넌트는 제거됨 — 화면 chrome 은 목업 html.) `intent='admin' + brand` 면 라우터도 antd 가 아니라 html/DS 로 보낸다(캐포비는 CMS 발화도 우회). 캐포비는 자체 admin DS(sidebar 300px · admin 토큰) — `:root[data-brand='cashwalk-biz']` cascade 로 brand-subtle bg + 노란 indicator 가 자동 적용, 색 hex 박지 말 것.",
    ...
    "recommended": [
      "**캐포비 어드민이면 ready-made 픽업**: items 를 손으로 만들지 말고 `get_guide({ topic: 'pattern:cashwalk-biz-admin-sidebar' })` 의 복붙 HTML 트리(아이콘 inline 완료)를 쓰고 active-key 만 화면 키로. BrandHeader/Footer 처럼 한 번에 끌어온다.",
...
export const PATTERN_GUIDES: Record<string, PatternGuide> = {
  ...
  "cashwalk-biz-admin-sidebar": {
    ...
    "summary": "**[캐포비 어드민 사이드바 ready-made]** cashwalk-biz admin LNB 를 한 번에 픽업하는 SSOT. 목업 전용 `<nds-sidebar>`(HTML, nds-brand-chrome 흡수) 컴포넌트는 완성돼 있지만 BrandHeader/Footer 와 달리 메뉴 데이터가 안 박혀 있어 매번 손조립하던 마찰을 해소한다. 헤더 블록(로고→계정→잔액→충전/내역 CTA) + 3섹션(광고 관리 / 자산 관리 / 계정 관리) + GNB 아이콘 9종이 들어간 메뉴 트리를 그대로 복붙. 활성 bg(Yellow/100)·radius(16)·텍스트색은 `data-brand='cashwalk-biz'` cascade 로 자동 — 색 hex 박지 말 것. 컴포넌트 props 함정은 `get_guide({ topic:'component:Sidebar', brand:'cashwalk-biz' })`, shell 조립은 `pattern:admin-shell`.",
    "rules": [
      ...
      "**HTML 복붙 (목업)**: `_readyMade.html` 트리를 그대로 복붙(brand= 로고 자동주입, item.icon inline SVG 완료 — find_icon 9회 불필요, items/account/footer-actions 는 `<script type=\"application/json\" slot=\"...\">` 텍스트 노드). active-key 만 변경. (공개 react `Sidebar` 컴포넌트는 제거됨 — admin 사이드바는 목업 html 전용.)",
  ...
  "geniet-screens": {
    "name": "geniet-screens",
    "metrics": {
      "brand": "geniet (헬시밀·다이어트 식단 커머스 — 샐러드/도시락)",
      "homeWeb": "데스크탑 — 상단 글로벌 nav + 검색 → cream 히어로 프로모 배너 → '랭킹' 음식 카드 그리드(4-up 행, 썸네일+제목+가격+브랜드 배지) 섹션 반복 → 전문가 칼럼/매거진 카드 → 푸터",
      ...
      "card": "음식 카드 = 정사각 썸네일 + 제목 2줄 + 가격 + 브랜드/할인 배지. 그리드는 4-up(web) 반복 랭킹 섹션.",
      "accent": "청록(teal) 브랜드 컬러 — 로고/CTA/링크. 식품 사진이 주인공이라 UI 채도는 낮게.",
      "status": "스크린샷 레퍼런스 카탈로그 (레이아웃·구성 SSOT). 정확한 색/여백/치수 토큰은 component 가이드 + 브랜드 토큰이 SSOT."
    },
    "references": [
      {
    ...
    "summary": "Geniet(헬시밀·다이어트 식단 커머스) 화면의 **시각 레퍼런스 카탈로그** — 웹 홈 / 앱 리뷰 상세 / 웹 리뷰 작성 모달 3종 스크린샷을 담는다. Geniet 은 샐러드·도시락 같은 식품 사진이 주인공이라 UI 채도는 낮추고 청록(teal) 브랜드 컬러를 로고·CTA·링크에만 쓴다. 화면을 새로 짤 때 **레이아웃·구성·위계의 출처**로 이 스크린샷들을 먼저 보고(`references[].imageAbsolutePath`), 정확한 색·여백·치수·컴포넌트 선택은 해당 `component:*` 가이드와 브랜드 토큰을 SSOT 로 따른다.",
    "rules": [
      "**음식 카드 그리드 = '카드 1장 = 상품 1건'.** 정사각 썸네일 + 제목(2줄) + 가격 + 브랜드/할인 배지. 웹 홈은 이 카드를 4-up 행으로 묶은 '랭킹' 섹션을 반복한다 → 섹션 컨테이너는 헤더(섹션명 + 더보기)를 소유하는 `pattern:card-section` / `List` 정합.",
      ...
      "**브랜드 컬러는 토큰으로만.** 청록 accent 는 `--semantic-*`(brand) 토큰 cascade 로 흐른다 — 컴포넌트/목업에 hex 하드코딩 금지(`get_guide({ topic: 'principles' })`).",
    ...
    "avoid": [
      "스크린샷의 색·여백·치수를 픽셀 단위로 그대로 베끼기 — 이 카탈로그는 **레이아웃·구성 참고용**이고, 정확한 값은 component 가이드 + 브랜드 토큰이 SSOT(어긋나면 토큰이 우선).",
      ...
      "Geniet 전용이라며 컴포넌트에 `[data-brand=\"geniet\"]` 색 분기 추가 — 브랜드 차이는 브랜드 토큰 파일에서 값만 override(`CLAUDE.md` 브랜드 분기 금지 규칙)."
    ]
  },
  "host-spacing": {
    "name": "host-spacing",
  ...
  "html-mockup-intake": {
    ...
    "summary": "기존 HTML 목업(또는 HTML 형식의 기획서)이 입력으로 들어왔을 때, 그것을 '재설계 대상'이 아니라 '재현할 정답 디자인'으로 보고 구조·콘텐츠·문구·위계를 보존한 채 primitive 만 DS(<nds-*> + 시멘틱 토큰 + 브랜드 크롬)로 바꿔 충실히 옮기는 패턴.",
    "rules": [
  ...
  "iconography": {
    ...
    "metrics": {
      ...
      "tracks": "mono (2114, currentColor) | multicolor (61, style-locked brand signature)",
    ...
    "rules": [
      ...
      "**명명 규칙 SSOT = `packages/icons/README.md` \"명명 규칙\" 섹션** (하드 게이트 `check-icon-naming.mjs` 가 kebab·철자·`-solid`·`*-circle` 어순을 강제). 파일명 = export 이름이라 rename 은 외부 API breaking → changeset(minor) 필수. 패턴 `icon-{brand?}-{name}-{modifier?}` · 어순 **명사-수식어**(`check-circle`, ❌`circle-check`) · 도형 **화살표(`arrow-*`) ≠ 셰브론(`chevron-*`)**, 기능명(`back`/`next`) 금지 · 철자 올바른 영어(`alarm`·`location`·`image`, 검색 클리어=`search-delete`).",
      "**브랜드 아이콘은 자기 슬러그 prefix 로 자기 세트를 유지**(`runmile-home`·`runmile-search` 등) — 범용 공유 아이콘(`home`/`search`/`calendar`)과 글리프가 달라 prefix 를 떼면 다른 브랜드의 공용 아이콘을 덮어써 시각 회귀가 난다. 그래서 브랜드 UI 아이콘을 \"범용화(prefix 제거)\" 하지 않는다. 시그니처(`runmile-shoe`·`runmile-shoe-color`)도 prefix 유지.",
      "**채움/상태 접미사는 레포 SSOT(축 분리)**: 시각 채움 = `-solid`/`-line` · 폼/토글 = `-on`/`-off` · 네비 선택 = `-active`. ⚠ 런마일 Figma 아이콘 네이밍 가이드([node 5016:43](https://www.figma.com/design/MssCIDnDdAjStQXHclPNIc/?node-id=5016-43))는 채움 `-fill` + 상태 전부 `-active`/`-inactive` 통일을 제안하나, **레포 명명 SSOT(노션 미러)가 governing** 이라 `-solid` + 축 분리를 유지한다. 5016:43 은 런마일 브랜드 아이콘 8 카테고리의 설계 근거 레퍼런스로 본다.",
      ...
      "**Mono vs Multicolor 트랙 분리** (SEED 스타일) — `@nudge-design/icons` 는 두 트랙으로 나뉜다. Mono(2114개, `currentColor` 만 사용해서 `color` prop 으로 자유 변경) 와 Multicolor(61개, 브랜드 시그니처 — Trost mental 일러스트·Trost 결과 아이콘(TestresultSafe/Warning/Danger)·rating StarHalf·MenuActive·AlarmDot·Book·Medal·EnergyCoin·Geniet 브랜드 아이콘·CashwalkBiz GNB chat/member 등 — 내부 accent 색 잠금). UI chrome(navigation/action/state)은 mono, 서비스 시그니처/주요 진입점은 multicolor.",
  ...
  "runmile-screens": {
    ...
    "metrics": {
      "brand": "runmile (러닝 대회·레이스 모집/참가 + 러너 커뮤니티)",
    ...
    "rules": [
      ...
      "**브랜드 컬러는 토큰으로만.** 코랄·핑크 accent 는 `--semantic-*`(brand) 토큰 cascade 로 흐른다 — 컴포넌트/목업에 hex 하드코딩 금지(`get_guide({ topic: 'principles' })`).",
    ...
    "avoid": [
      ...
      "Runmile 전용이라며 컴포넌트에 `[data-brand=\"runmile\"]` 색 분기 추가 — 브랜드 차이는 브랜드 토큰 파일에서 값만 override(`CLAUDE.md` 브랜드 분기 금지 규칙)."
    ]
  },
  "scroll-rail": {
    "name": "scroll-rail",
  ...
  "surface-layer": {
    ...
    "summary": "Surface / Background 의 4단계 레이어 정의와 Brand background 사용 원칙. Brand background 는 시각 장식이 아니라 '의미 전달'(주의·안내·강조) 목적으로만 사용한다. notice 패턴과 짝.",
    "rules": [
  ...
  "trost-screens": {
    "name": "trost-screens",
    "metrics": {
      "brand": "trost (심리상담·멘탈케어 EAP — 상담/콘텐츠/칼럼)",
      ...
      "status": "스크린샷 레퍼런스 카탈로그 (레이아웃·구성 SSOT). Trost 웹은 Zeplin 이 디자인 SSOT — 정확한 색/여백/치수는 Zeplin + 브랜드 토큰."
    },
    ...
    "summary": "Trost(심리상담·멘탈케어 EAP) 웹 홈의 **시각 레퍼런스 카탈로그** — 사진 배경 다크 히어로 + 콘텐츠/칼럼 카드 그리드 구조의 데스크탑 홈 스크린샷을 담는다. Trost 웹은 **Zeplin 이 디자인 SSOT**(zpl.io/Dp775xl)이고, 블루 브랜드 컬러를 로고·CTA·링크·배너에만 쓴다. 화면을 새로 짤 때 **레이아웃·구성·위계의 출처**로 이 스크린샷을 먼저 보고(`references[].imageAbsolutePath`), 정확한 색·여백·치수·컴포넌트 선택은 Zeplin 과 브랜드 토큰을 SSOT 로 따른다.",
    "rules": [
      ...
      "**브랜드 컬러는 토큰으로만.** 블루 accent 는 `--semantic-*`(brand) 토큰 cascade 로 흐른다 — 컴포넌트/목업에 hex 하드코딩 금지.",
    ...
    "avoid": [
      "스크린샷의 색·여백·치수를 픽셀 단위로 그대로 베끼기 — 이 카탈로그는 **레이아웃·구성 참고용**이고, 정확한 값은 Zeplin + 브랜드 토큰이 SSOT(어긋나면 SSOT 우선).",
      ...
      "Trost 전용이라며 컴포넌트에 `[data-brand=\"trost\"]` 색 분기 추가 — 브랜드 차이는 브랜드 토큰 파일에서 값만 override(`CLAUDE.md` 브랜드 분기 금지 규칙)."
    ]
  },
