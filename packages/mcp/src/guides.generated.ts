...
export const PATTERN_GUIDES: Record<string, PatternGuide> = {
  ...
  "react-migration": {
    ...
    "rules": [
      ...
      "**흐름 예**: ① `nds-migrate src/**/*.tsx`(button/input/badge/chip/textarea 결정적 교체 + import 자동 병합) → ② 남은 Modal·checkbox 는 `get_guide({ topic:'component:Modal' })` 보고 손 조립 → ③ `tsc --noEmit`. ❌ Modal·checkbox 를 코드모드에 욱여넣지 말 것(onChange 시그니처·compound 구조가 깨진다)."
    ]
  },
  "selection-controls": {
