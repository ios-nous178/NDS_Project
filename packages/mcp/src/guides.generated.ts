...
export const COMPONENT_GUIDES: Record<string, ComponentGuide> = {
  ...
  "Badge": {
    ...
    "summary": "상태/속성을 한눈에 알려주는 보조 라벨. variant: fill/ghost/line · color: brand/neutral/success/error/caution/info. Figma 171:10856. label prop 필수. 콘텐츠가 아니라 콘텐츠를 보조하는 메타 정보만 담는다.",
    "pitfalls": [
  ...
  "CheckboxGroup": {
    "name": "CheckboxGroup",
    "summary": "체크박스 묶음. 두 모드 — **items(데이터 모드)**: value/onValueChange 로 선택 관리 + `select-all`(자식 비율로 indeterminate 자동) + `badge`(필수/선택 등)·`detail`(펼침) 슬롯. **children(레이아웃 모드)**: 직접 조립한 nds-checkbox 들을 배치만. '전체선택 + 체크 리스트'(약관 동의·다중 필터·설정 묶음)의 단일 컴포넌트. antd Checkbox.Group 대응.",
    "pitfalls": [
  ...
  "CountdownTimer": {
    "name": "CountdownTimer",
    "summary": "종료 시각까지 자동 카운트다운. 1초 단위 갱신, 10초 이하 빨강 강조, mm:ss/hh:mm:ss/remaining 포맷. **tone=\"brand\"** 면 진행 중 시간을 브랜드 액센트색(캐포비=오렌지 #FD9B02)으로 — 인증 입력의 타이머처럼 강조할 때. (urgent ≤10s 빨강은 tone 과 무관하게 우선.)",
    "pitfalls": [
  ...
  "EmptyState": {
    "name": "EmptyState",
    "summary": "데이터/검색 결과/기록 없음 표시. 단순 '없음' 메시지 대신 다음 액션(추가하기 / 다시 검색 / 추천 보기)을 제안.",
    "pitfalls": [
    ...
    "matrixOverrides": {
      "cashwalk-biz": {
        "dimensions": {
          ...
          "buttonHeight": "44px pill (base 변형 없음) — 모달 액션 버튼 shape 는 pill 이 맞음 (Figma ModalGuide 3418-471). default 사각으로 바꾸지 말 것.",
          "footerLayout": "Single(확정 1개) = 우측 정렬 · hug 너비(footer-action 구조면 120px 고정) · 검정 pill — **full-width 아님**. Dual(취소+확정) = 가로 분할. HTML 은 `<div slot=\"footer\">` 로 감싸면 자동으로 .nds-modal__footer 로 승격돼 이 레이아웃이 적용됨(버튼 2개면 data-has-both-actions=\"true\" 자동). single 에 full-width 를 붙이거나 footer 컨테이너 없이 버튼만 두면(본문 가운데 끼임) 회귀.",
  ...
  "MultiStepForm": {
    "name": "MultiStepForm",
    "summary": "다단계 폼 컨테이너. 단계별 검증/진행/제출을 한 컴포넌트에서 관리. Stepper(인디케이터만)와 분리.",
    "pitfalls": [
  ...
  "PageHeader": {
    "name": "PageHeader",
    "summary": "페이지 단위 헤더. 제목 + 서브타이틀 + 우측 액션 + 하단 탭 슬롯. AppBar(글로벌 네비)와 분리.",
    "pitfalls": [
  ...
  "PinPad": {
    "name": "PinPad",
    "summary": "PIN 키패드. 점 인디케이터 + 숫자 그리드. SMS 인증은 VerificationCodeInput, 일반 입력은 Input.",
    "pitfalls": [
  ...
  "QuickMenu": {
    ...
    "pitfalls": [
      ...
      "**icon = inline SVG 문자열 (이름/이모지 아님).** `icon` 은 innerHTML 로 주입되므로 `\"icon\":\"home\"` 같은 이름/이모지를 넣으면 텍스트로 흘러나온다. `find_icon({ name })` → 반환 inline SVG 를 넣는다. Icon Library 의 32px line style 로 통일. React `QuickMenu` 의 `icon: ReactNode` 와 대칭, nds-quick-action-grid 와 동일 규약.",
  ...
  "Sidebar": {
    ...
    "interactivePattern": "활성 키 관리는 호스트 라우터에서 결정 (activeKey={location.pathname}). onItemClick 은 navigation 트리거용.",
    "examplesHtml": {
  ...
  "SignaturePad": {
    "name": "SignaturePad",
    "summary": "전자 서명 캔버스. 동의서/가입 서명 추출. ref(SignaturePadHandle)로 clear/toDataURL/isEmpty.",
    "pitfalls": [
  ...
  "Timeline": {
    "name": "Timeline",
    "summary": "타임라인 — data-array(items) API. mode=activity(시간순 이벤트 로그 — date/title/description + status/statusLabel 배지) + mode=tracker(정해진 단계 진행 트래커 — current 인덱스로 done/current/todo 파생, direction=vertical|horizontal). 구 ActivityTimeline+StatusTimeline 통합. (단계 진척만 간결히 보이려면 Stepper, 다단계 폼 컨테이너는 MultiStepForm.)",
    "pitfalls": [
  ...
  "TitleGroup": {
    ...
    "summary": "헤딩 + 서브타이틀 표준 블록. level (h1~h5) 만 결정하면 헤딩 폰트와 Gap/Title 토큰이 자동 적용 — Figma TitleGapGuide 859:5614 (6 페이지 58건 실측) 기반.",
    "pitfalls": [
  ...
  "UserCard": {
    "name": "UserCard",
    "summary": "범용 프로필 미니카드. row/stacked, verified, action 슬롯. CounselorCard(EAP 특화)와 분리.",
    "pitfalls": [
  ...
  "VoiceRecorder": {
    "name": "VoiceRecorder",
    "summary": "음성 메모 녹음 UI. 큰 녹음 버튼 + 타이머 + 펄스 인디케이터. 마이크 접근/저장은 외부 처리.",
    "pitfalls": [
...
export const PATTERN_GUIDES: Record<string, PatternGuide> = {
  ...
  "cashwalk-biz-admin-sidebar": {
    ...
    "summary": "**[캐포비 어드민 사이드바 ready-made]** cashwalk-biz admin LNB 를 한 번에 픽업하는 SSOT. `<Sidebar>`(React) / `<nds-sidebar>`(HTML) 컴포넌트는 완성돼 있지만 BrandHeader/Footer 와 달리 메뉴 데이터가 안 박혀 있어 매번 손조립하던 마찰을 해소한다. 헤더 블록(로고→계정→잔액→충전/내역 CTA) + 3섹션(광고 관리 / 자산 관리 / 계정 관리) + GNB 아이콘 9종이 들어간 메뉴 트리를 그대로 복붙. 활성 bg(Yellow/100)·radius(16)·텍스트색은 `data-brand='cashwalk-biz'` cascade 로 자동 — 색 hex 박지 말 것. 컴포넌트 props 함정은 `get_guide({ topic:'component:Sidebar', brand:'cashwalk-biz' })`, shell 조립은 `pattern:admin-shell`.",
    "rules": [
    ...
    "_readyMade": {
      ...
      "react": "import { CashwalkBizGnbBannerIcon, CashwalkBizGnbChannelIcon, CashwalkBizGnbQuizIcon, CashwalkBizGnbChatIcon, CashwalkBizGnbCashIcon, CashwalkBizGnbCatalogIcon, CashwalkBizGnbMemberIcon, CashwalkBizGnbEditIcon, CashwalkBizGnbSettingIcon } from \"@nudge-design/icons\";\nimport { Sidebar } from \"@nudge-design/react\";\n\nconst items = [\n  {\n    \"key\": \"ad\",\n    \"label\": \"광고 관리\",\n    \"items\": [\n      {\n        \"key\": \"banner\",\n        \"label\": \"배너\",\n        \"icon\": <CashwalkBizGnbBannerIcon />,\n        \"children\": [\n          {\n            \"key\": \"banner-new\",\n            \"label\": \"배너 등록\"\n          },\n          {\n            \"key\": \"banner-list\",\n            \"label\": \"배너 목록\"\n          },\n          {\n            \"key\": \"banner-report\",\n            \"label\": \"배너 리포트\"\n          }\n        ]\n      },\n      {\n        \"key\": \"channel\",\n        \"label\": \"채널\",\n        \"icon\": <CashwalkBizGnbChannelIcon />\n      },\n      {\n        \"key\": \"quiz\",\n        \"label\": \"퀴즈\",\n        \"icon\": <CashwalkBizGnbQuizIcon />\n      },\n      {\n        \"key\": \"message\",\n        \"label\": \"메시지\",\n        \"icon\": <CashwalkBizGnbChatIcon />\n      }\n    ]\n  },\n  {\n    \"key\": \"asset\",\n    \"label\": \"자산 관리\",\n    \"items\": [\n      {\n        \"key\": \"cash\",\n        \"label\": \"캐시 충전\",\n        \"icon\": <CashwalkBizGnbCashIcon />\n      },\n      {\n        \"key\": \"catalog\",\n        \"label\": \"상품 카탈로그\",\n        \"icon\": <CashwalkBizGnbCatalogIcon />\n      }\n    ]\n  },\n  {\n    \"key\": \"account\",\n    \"label\": \"계정 관리\",\n    \"items\": [\n      {\n        \"key\": \"member\",\n        \"label\": \"회원\",\n        \"icon\": <CashwalkBizGnbMemberIcon />\n      },\n      {\n        \"key\": \"content\",\n        \"label\": \"콘텐츠 편집\",\n        \"icon\": <CashwalkBizGnbEditIcon />\n      },\n      {\n        \"key\": \"setting\",\n        \"label\": \"설정\",\n        \"icon\": <CashwalkBizGnbSettingIcon />\n      }\n    ]\n  }\n];\n\n// 로고 아래 고정 계정 블록 + 푸터 로그아웃 — HTML <nds-sidebar account/footer-actions> 와 미러.\nconst account = {\n  email: \"biz@cashwalk.io\",\n  balanceLabel: \"충전 잔액\",\n  balance: \"₩1,250,000\",\n  actions: [\n    { key: \"charge\", label: \"충전하기\", variant: \"solid\" as const, onClick: () => openCharge() },\n    { key: \"history\", label: \"내역보기\", variant: \"outlined\" as const, onClick: () => openHistory() },\n  ],\n};\n\n<Sidebar items={items} activeKey=\"banner-list\" width={300}\n  logo={{ src: \"/brand-logos/cashwalk-biz.svg\", alt: \"Cashwalk for Business\" }}\n  account={account}\n  footerActions={[{ key: \"logout\", label: \"로그아웃\", variant: \"outlined\", onClick: () => logout() }]}\n  onItemClick={(it) => navigate(it.key)} />",
  ...
  "visual-reference": {
    ...
    "referenceInputs": {
      ...
      "fallbackQuestion": "시각 기준으로 쓸 Figma 링크나 스크린샷이 있을까요? 이미 첨부하신 자료를 기준으로 진행해도 될지, 추가로 정답/오답 레퍼런스가 있으면 함께 알려 주세요. 가능하면 정답 1-2장, 피해야 할 오답 1-2장에 각각 1줄 캡션을 붙여 주세요."
    },
    "examples": [
      {
        "verdict": "good",
