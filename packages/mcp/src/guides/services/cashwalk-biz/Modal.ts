 ...
 * Source: legacy COMPONENT_GUIDES.Modal.projectMatrix['cashwalk-biz'] (제거됨, 이쪽으로 마이그레이션) + Modal.pitfalls 의 캐시워크 포 비즈니스 한정 2건.
 *
 * 빠진 내용 (의도적, 새 모델 분리):
 *   - 480/16/32/gap20/typography 같은 spec 차이 → COMPONENT_GUIDES.Modal.matrixOverrides['cashwalk-biz'].dimensions (overlay 아닌 base 안 project-aware metadata).
 *   - 색 정보 (#000 검정 CTA 등) → 토큰 SSOT (packages/tokens/src/projects/cashwalk-biz.semantic.ts) 가 담당.
 */
export const ModalOverlay: ServiceOverlay = {
  ...
  servicePitfalls: [
    ...
    'admin 모달이라고 가정해 너비/패딩/라운드를 inline style 로 덮어쓰지 말 것. `<html data-project="cashwalk-biz">` 가 박힌 환경이면 480/16/32 가 자동 적용 — 그 외 컨텍스트라면 base 모바일 스펙이 의도된 것.',
  ],
  forbiddenPatterns: [
