 ...
 *
 * Sidebar 는 admin 전용 컴포넌트로 사실상 cashwalk-biz brand 가 주 사용자. gnb 키: 좌측 메뉴 아이콘 자리 — brand prefix 아이콘 7종이 같은 의미의 공용 아이콘보다 우선.
 *
 * 빠진 내용 (의도적, SSOT 분리):
 *   - 활성 아이템 bg(Yellow/100 #FFFAE5) 는 시멘틱 토큰 SSOT — cashwalk-biz.semantic.ts bg.brand.subtle → --semantic-bg-brand-subtle cascade. 컴포넌트/overlay 가 hex 로 박지 않음.
 *   - 컴포넌트 brand preset(Sidebar.tsx sidebarCashwalkBizTuning) 에는 시멘틱 색 토큰으로 표현 불가능한 것만: active radius 16(geometry), 선택 시 라벨 normal(#333) 유지(text role 선택).
...
export const SidebarOverlay: ServiceOverlay = {
  ...
  preferredPatterns: [
    "상단은 brand 로고 → 계정 정보(이메일) → 잔액(충전 금액) → CTA 쌍(충전하기 solid / 내역보기 outlined) 순의 고정 헤더 블록 — 잔액·충전은 캐포비 admin 의 핵심 메타라 메뉴 위에 항상 노출.",
  ...
  servicePitfalls: [
    "캐포비 admin 의 좌측 GNB 아이콘은 brand prefix(CashwalkBizGnb*) 가 공용 아이콘보다 우선 — 배너=CashwalkBizGnbBannerIcon, 퀴즈=CashwalkBizGnbQuizIcon, 메시지=CashwalkBizGnbChatIcon 등.",
    "충전하기(solid)·내역보기(outlined) 는 동일 너비 2-up 버튼 쌍 — 한쪽만 강조하거나 세로로 쌓지 말 것.",
    "활성 배경(Yellow/100)·radius(16)·텍스트색은 data-brand='cashwalk-biz' cascade 로 자동 — HTML/inline 으로 다시 박지 말 것.",
  ],
};
