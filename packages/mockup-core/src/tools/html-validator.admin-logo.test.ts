 ...
 * 내장돼 있는데도). admin 셸 사이드바·톱바에 손수 만든 로고가 있고 <nds-sidebar brand> /
 * <nds-brand-logo> 가 없으면 warn 으로 컴포넌트 사용을 유도. 사이드바 아이콘(inline SVG)·
 * 계정 아바타는 오탐 제외.
 */
...
const PROPER_SIDEBAR_BRAND = `<aside class="nds-shell__sidebar">
  <nds-sidebar brand="geniet" items="[]"></nds-sidebar>
</aside>`;

const PROPER_BRAND_LOGO = `<aside class="nds-shell__sidebar">
  <nds-brand-logo brand="geniet" height="40"></nds-brand-logo>
  <nds-sidebar items="[]"></nds-sidebar>
</aside>`;

const SIDEBAR_ICONS_ONLY = `<aside class="nds-shell__sidebar">
  <nds-sidebar brand="geniet" items='[{"label":"홈","icon":"<svg viewBox=\\"0 0 24 24\\"><path d=\\"M1 1\\"/></svg>"}]'></nds-sidebar>
</aside>`;
...
test("<nds-sidebar brand> 로 로고 자동 주입 → 위반 아님", () => {
  const v = validateHtmlSource(PROPER_SIDEBAR_BRAND);
  assert.equal(v.find((x) => x.rule === RULE), undefined);
});

test("<nds-brand-logo> 사용 → 위반 아님", () => {
  const v = validateHtmlSource(PROPER_BRAND_LOGO);
  assert.equal(v.find((x) => x.rule === RULE), undefined);
});

test("사이드바 아이콘(inline SVG)만 있고 로고는 brand 주입 → 위반 아님(오탐 방지)", () => {
