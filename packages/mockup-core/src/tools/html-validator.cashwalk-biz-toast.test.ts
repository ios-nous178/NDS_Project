 ...
 * cashwalk-biz-toast 룰로 전면 차단(error)한다.
 */

test("캐포비에서 nds-toast 는 cashwalk-biz-toast error 로 잡는다", () => {
  const v = validateHtmlSource(
    `<html data-brand="cashwalk-biz"><body><nds-toast message="저장됨"></nds-toast></body></html>`,
  );
  const hit = v.find((x) => x.rule === "cashwalk-biz-toast");
  assert.ok(hit, "cashwalk-biz-toast 위반이 있어야 함");
...
test("캐포비 별칭(cashpobi)에서도 nds-toast 를 잡는다", () => {
  ...
  assert.ok(v.find((x) => x.rule === "cashwalk-biz-toast"));
});
...
test("캐포비에서 nds-snackbar 는 위반이 아니다", () => {
  ...
  assert.equal(
    v.find((x) => x.rule === "cashwalk-biz-toast"),
    undefined,
  );
});

test("다른 브랜드(미선언)에서는 nds-toast 가 정상 — 룰 미적용", () => {
  const v = validateHtmlSource(`<html><body><nds-toast message="저장됨"></nds-toast></body></html>`);
  assert.equal(
    v.find((x) => x.rule === "cashwalk-biz-toast"),
    undefined,
  );
