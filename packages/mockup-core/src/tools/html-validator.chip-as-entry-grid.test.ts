...
const WRONG_ICON_TILES = `<html data-project="geniet"><body>
  <section>
...
const WRONG_GRID_STYLE = `<html data-project="geniet"><body>
...
const CORRECT_QA_GRID = `<html data-project="geniet"><body>
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px">
...
const OK_TEXT_SELECT_CHIPS = `<html data-project="geniet"><body>
...
const OK_FEW_CHECK_ICONS = `<html data-project="geniet"><body>
  <div style="display:flex;flex-wrap:wrap;gap:8px">
...
test("아이콘 단 chip ≥6 을 한 그룹으로 나열 → chip-as-entry-grid warn", () => {
  const v = validateHtmlSource(WRONG_ICON_TILES, { surface: "service", project: "geniet" });
...
test("CSS 그리드 레이아웃에 chip ≥6 → chip-as-entry-grid warn", () => {
  const v = validateHtmlSource(WRONG_GRID_STYLE, { surface: "service", project: "geniet" });
  assert.ok(has(v, "chip-as-entry-grid"), "그리드에 chip 다수면 위반이어야 함");
});

test("chip-as-entry-grid 는 D1 layout 점수에도 반영", () => {
  const result = validateHtmlMockup({ source: WRONG_ICON_TILES, surface: "service", project: "geniet" });
...
test("정답(quick-action-grid Card 셀)은 chip-as-entry-grid 위반 없음", () => {
  const v = validateHtmlSource(CORRECT_QA_GRID, { surface: "service", project: "geniet" });
  assert.equal(has(v, "chip-as-entry-grid"), undefined, "Card 셀 그리드는 chip 이 아니므로 통과");
});

test("텍스트-only 다중선택 칩(연령대)은 오탐 없음", () => {
  const v = validateHtmlSource(OK_TEXT_SELECT_CHIPS, { surface: "service", project: "geniet" });
  assert.equal(has(v, "chip-as-entry-grid"), undefined, "그리드도 아이콘 타일도 아니면 통과");
});

test("선택된 1~2개만 체크 아이콘 단 다중선택 칩은 오탐 없음", () => {
  const v = validateHtmlSource(OK_FEW_CHECK_ICONS, { surface: "service", project: "geniet" });
