...
const hitCashwalk = (html: string, rule: string) => vCashwalk(html).find((x) => x.rule === rule);
...
test("add 어포던스 2개(외부 추가 + 패널 '추가 선택') → selected-item-add-affordance-duplicated", () => {
  ...
  assert.equal(hasCashwalk(html, "selected-item-add-affordance-duplicated"), true);
});
...
test("add 어포던스 중복은 error(빌드 게이트 차단) — 재발 회귀", () => {
  ...
  assert.equal(hitCashwalk(html, "selected-item-add-affordance-duplicated")?.severity, "error");
});

test("캐포비 scope 밖 add 어포던스 중복은 위반 아님", () => {
  const html = `<html><body>
    <nds-button>+ 항목 추가</nds-button>
    <nds-selected-items-panel>
      <nds-button>+ 추가 선택</nds-button>
...
test("캐포비 add 어포던스 1개는 위반 아님", () => {
  const html = `<html><body>
    <nds-selected-items-panel><nds-button>+ 추가 선택</nds-button></nds-selected-items-panel>
  </body></html>`;
  assert.equal(hasCashwalk(html, "selected-item-add-affordance-duplicated"), false);
});
...
test("캐포비 페이지 패널 1개(hide-add 없음)만 있으면 정답 — 미발화(오탐 방지)", () => {
  const html = `<html><body>
    <nds-selected-items-panel>
      <nds-selected-item-row>서울특별시 &gt; 강남구</nds-selected-item-row>
    </nds-selected-items-panel>
...
test("선택 모달에 SelectedItemsPanel 없음 → selected-items-modal-missing-panel", () => {
  ...
  assert.equal(hasCashwalk(html, "selected-items-modal-missing-panel"), true);
});

test("캐포비 scope 밖 지역 모달 시그니처는 selected-items-modal-missing-panel 위반 아님", () => {
  const html = `<html><body>
    <nds-modal>
      <div>시/도, 시/군/구를 순서대로 선택한 뒤 [적용]을 누르세요</div>
      <ul><li>서울특별시</li></ul><ul><li>강남구</li></ul>
...
test("캐포비 우측 SelectedItemsPanel 갖춘 2단 모달은 위반 아님(오탐 방지)", () => {
  const html = `<html><body>
    <nds-modal>
  ...
  assert.equal(hasCashwalk(html, "selected-items-modal-missing-panel"), false);
});
...
// ── 캐포비 성별 타겟팅 ────────────────────────────────────────
test("캐포비 성별 필드를 Select 로 구현 → cashwalk-biz-gender-selection-control", () => {
  const html = `<html><body>
    <div class="nds-form-row">
      <label class="nds-form-row__label">성별</label>
      <div class="nds-form-row__control">
        <nds-select placeholder="성별 선택"></nds-select>
      </div>
    </div>
  </body></html>`;
  const found = hitCashwalk(html, "cashwalk-biz-gender-selection-control");
  assert.ok(found);
  assert.match(found!.suggestion ?? "", /nds-selection-button-group/);
});

test("캐포비 성별 SelectionButtonGroup + selected Chip 조합은 위반 아님", () => {
  const html = `<html><body>
    <div class="nds-form-row">
      <label class="nds-form-row__label">성별</label>
      <div class="nds-form-row__control">
        <nds-selection-button-group value="custom" options='[{"value":"all","label":"전체"},{"value":"custom","label":"특정 성별"}]'></nds-selection-button-group>
        <div>
          <nds-chip selected interactive>남성</nds-chip>
          <nds-chip selected interactive>여성</nds-chip>
        </div>
      </div>
    </div>
  </body></html>`;
  assert.equal(hasCashwalk(html, "cashwalk-biz-gender-selection-control"), false);
});

test("캐포비 scope 밖 성별 Select 는 cashwalk-biz-gender-selection-control 위반 아님", () => {
  const html = `<html><body>
    <div class="nds-form-row">
      <label class="nds-form-row__label">성별</label>
      <div class="nds-form-row__control"><nds-select></nds-select></div>
    </div>
  </body></html>`;
  assert.equal(has(html, "cashwalk-biz-gender-selection-control"), false);
});
