 ...
 *  - project-modal-confirm-cta    : 확인/팝업 모달 footer 주 action 이 primary(노랑)/색생략(=기본 primary)
 *  - project-modal-footer-stacked : footer 두 버튼 세로 스택 (가로 유지 + 라벨 축약이 원칙)
...
// ─── primary CTA ─────────────────────────────────────────────────────────────
test("스크린샷 재현: 확인 모달 단일 버튼 color 생략 → project-modal-confirm-cta error (기본값 primary)", () => {
  ...
  const hit = has(v, "project-modal-confirm-cta");
...
test("확인 모달 명시적 color=\"primary\" 도 잡는다", () => {
  ...
  assert.ok(has(v, "project-modal-confirm-cta"));
});
...
test("검정 CTA(color=\"neutral\") 는 위반 아님", () => {
  ...
  assert.equal(has(v, "project-modal-confirm-cta"), undefined);
});
...
test("캐포비 아닌 프로젝트 모달은 primary 가 정상 — 미발화", () => {
  const v = validateHtmlSource(
    `<html data-project="trost"><body><nds-modal open><p>x</p>` +
      `<div slot="footer"><nds-button>확인</nds-button></div></nds-modal></body></html>`,
    { surface: "service", project: "trost" },
  );
  assert.equal(has(v, "project-modal-confirm-cta"), undefined);
});

// ─── footer 세로 스택 ────────────────────────────────────────────────────────
test("footer 2버튼 flex-direction:column → project-modal-footer-stacked warn", () => {
  const v = validateHtmlSource(
    doc(
      `<nds-modal open max-width="480"><p>x</p>` +
  ...
  const hit = has(v, "project-modal-footer-stacked");
...
test("가로(기본) footer 2버튼은 stacked 미발화", () => {
  ...
  assert.equal(has(v, "project-modal-footer-stacked"), undefined);
});
