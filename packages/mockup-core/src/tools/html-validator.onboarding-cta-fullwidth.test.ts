...
const onboarding = (btn: string) =>
  `<html data-brand="cashwalk-biz" data-page-pattern="onboarding"><body>` +
  `<nds-brand-logo brand="cashwalk-biz"></nds-brand-logo>${btn}</body></html>`;
...
test("온보딩이 아니면(패턴 미선언) 룰 미적용 — 모달 단일버튼 hug 와 충돌 안 함", () => {
  const v = validateHtmlSource(
    `<html data-brand="cashwalk-biz"><body><nds-button color="primary">확인</nds-button></body></html>`,
    OPTS,
  );
  assert.equal(
    v.find((x) => x.rule === "onboarding-cta-not-fullwidth"),
...
test("멀티스텝 푸터(이전 단계 + 제출, 카드 밖)는 제출 hug 를 면제한다", () => {
  const v = validateHtmlSource(
    `<html data-brand="cashwalk-biz" data-page-pattern="onboarding"><body>` +
      `<div class="onboarding-card"><nds-brand-logo brand="cashwalk-biz"></nds-brand-logo></div>` +
