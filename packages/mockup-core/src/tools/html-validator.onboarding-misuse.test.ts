 ...
 *   onboarding-missing-project-logo / onboarding-social-bare-text / onboarding-success-plain-circle /
...
function wrap(inner: string): string {
  return `<html data-project="cashwalk-biz" data-page-pattern="onboarding"><body>
...
// ─── 1) onboarding-missing-project-logo ───

test("로고를 raw svg 로 조립 + nds-project-logo 없음 → onboarding-missing-project-logo warn", () => {
  const html = wrap(`<svg viewBox="0 0 10 10"></svg><form><input placeholder="이메일" /></form>`);
  const hit = has(html, "onboarding-missing-project-logo");
  assert.ok(hit, "nds-project-logo 가 없으면 위반이어야 함");
  assert.equal(hit?.severity, "warn");
});

test("nds-project-logo 컴포넌트가 있으면 위반 아님", () => {
  const html = wrap(
    `<nds-project-logo project="cashwalk-biz" height="40"></nds-project-logo><form><input placeholder="이메일" /></form>`,
  );
  assert.equal(has(html, "onboarding-missing-project-logo"), undefined);
});
...
test("data-page-pattern=list 면 온보딩 룰 5종 전부 미적용", () => {
  const html = `<html data-project="cashwalk-biz" data-page-pattern="list"><body>
