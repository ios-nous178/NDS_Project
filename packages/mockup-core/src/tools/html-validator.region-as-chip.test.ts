...
const WRONG = `<html data-brand="cashwalk-biz"><body>
  <div class="field">지역
...
const CORRECT = `<html data-brand="cashwalk-biz"><body>
...
const PLAIN_CHIPS = `<html data-brand="cashwalk-biz"><body>
...
test("지역 경로(>)가 든 nds-chip → region-as-chip warn", () => {
  const v = validateHtmlSource(WRONG, { surface: "admin", brand: "cashwalk-biz" });
...
test("SelectedItemsPanel + RegionRow(정답)는 region-as-chip 위반 없음", () => {
  const v = validateHtmlSource(CORRECT, { surface: "admin", brand: "cashwalk-biz" });
  assert.equal(has(v, "region-as-chip"), undefined, "RegionRow 는 칩이 아니므로 통과");
});

test("일반 칩(지역 경로 없음)은 오탐 없음", () => {
  const v = validateHtmlSource(PLAIN_CHIPS, { surface: "admin", brand: "cashwalk-biz" });
