...
import { inferWorkspaceBrand } from "./build-html.js";
import { listStandaloneBrands } from "./standalone-assets.js";

const hasManifest = (() => {
  try {
    return listStandaloneBrands().includes("cashwalk-biz");
  } catch {
    return false;
  }
})();
...
test("brief.md '브랜드: X' 에서 추론한다", { skip: !hasManifest }, () => {
  withWorkspace("anything", { "brief.md": "브랜드: cashwalk-biz · 표면: admin" }, (cwd) => {
    assert.equal(inferWorkspaceBrand(cwd), "cashwalk-biz");
  });
});
...
test("brief 가 폴더명보다 우선한다", { skip: !hasManifest }, () => {
  withWorkspace("trost-screen-xyz", { "brief.md": "브랜드: cashwalk-biz" }, (cwd) => {
    assert.equal(inferWorkspaceBrand(cwd), "cashwalk-biz");
  });
});

test("브랜드 신호가 없으면 undefined", { skip: !hasManifest }, () => {
  withWorkspace("my-random-project", { "brief.md": "그냥 메모" }, (cwd) => {
    assert.equal(inferWorkspaceBrand(cwd), undefined);
  });
});
