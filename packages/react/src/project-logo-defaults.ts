/**
 * project-logo base64 data URI re-export.
 *
 * SSOT 는 `@nudge-design/assets/project-logo-defaults` — 변경 시 거기서만.
 * 이전엔 html/react 가 1:1 복제 상태로 drift 위험이 있어 통합.
 */
export {
  GENIET_LOGO_PC_DATA_URI,
  GENIET_LOGO_MOBILE_DATA_URI,
  GENIET_LOGO_FOOTER_DATA_URI,
  NUDGE_EAP_LOGO_DATA_URI,
  NUDGE_EAP_LOGO_FOOTER_DATA_URI,
  TROST_LOGO_DATA_URI,
  TROST_LOGO_MOBILE_DATA_URI,
  TROST_LOGO_WHITE_DATA_URI,
  CASHWALK_BIZ_LOGO_DATA_URI,
  RUNMILE_LOGO_DATA_URI,
  RUNMILE_LOGO_GRAY700_DATA_URI,
} from "@nudge-design/assets/project-logo-defaults";
