...
export const datePickerStyles = `
  ...
  /* HTML 어댑터(<nds-date-picker>) 는 빈 wrapper 만 두고 CSS mask 로 캘린더 글리프를 그린다.
     React 어댑터는 자식 <svg> 가 있어 :empty 매칭이 안 되므로 영향 없음 — useProject() 훅이 같은 역할을 수행한다.
     프로젝트 글리프를 추가할 때는 아래 [data-project="..."] cascade 에 한 줄만 더하면 된다. */
  ...
  :where([data-project="cashwalk-biz"] .${DP_ICON_CLASS}:empty) {
