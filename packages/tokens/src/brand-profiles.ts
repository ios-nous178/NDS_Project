/**
 * 브랜드 프로필 — 브랜드별 "의미/정책 차이"의 단일 수거처(SSOT).
 *
 * 배경: 검정 CTA 매핑(캐포비=neutral · Geniet=secondary), Toast 금지, 모달 confirm 색
 * 같은 브랜드 분기가 validator 하드코딩(`brand === "cashwalk-biz"`)·별칭맵·가이드 문자열에
 * 산재해 "한쪽만 고치는" 재발(캐포비 모달 노랑 5회+)의 토양이 됐다. 정책 **데이터**는
 * 여기로 모으고, validator/빌더는 브랜드 slug 를 하드코딩하지 않고 프로필을 읽는다.
 *
 * 목표: 6번째 브랜드 추가 = palette/semantic 토큰 + 이 파일 엔트리 1개.
...
export interface BrandCtaPolicy {
  /**
   * 검정(다크 인버스) CTA 슬롯이 매핑되는 Button color.
   * "neutral" 선언 브랜드는 neutral+solid 가 정당한 CTA — validator `neutral-solid-cta` 면제.
   */
  blackCta?: "neutral" | "secondary";
  /** 이 브랜드에서 금지되는 Button color → 대체 안내(validator suggestion 에 그대로 노출). */
  deniedButtonColors?: ReadonlyArray<{ color: string; useInstead: string }>;
}

export interface BrandModalPolicy {
  /**
...
export interface BrandProfile {
  ...
  cta?: BrandCtaPolicy;
  modal?: BrandModalPolicy;
  notifications?: BrandNotificationPolicy;
  admin?: {
    /**
...
export const BRAND_PROFILES: Record<BrandSlug, BrandProfile> = {
  trost: { slug: "trost" },
  geniet: {
    slug: "geniet",
    // Geniet Solid Secondary = gray/900 다크 인버스(다른 브랜드의 light subtle 과 다름).
...
export const BRAND_ALIAS_MAP: Readonly<Record<string, BrandSlug>> = Object.freeze(
  Object.fromEntries(
    (Object.values(BRAND_PROFILES) as BrandProfile[]).flatMap((p) =>
      (p.aliases ?? []).map((a) => [a.toLowerCase(), p.slug]),
    ),
  ) as Record<string, BrandSlug>,
);

/** 입력(정식 slug 또는 별칭)을 정식 slug 로. 미지 입력은 undefined — 폴백/경고는 호출부 책임. */
export function resolveBrandSlug(input?: string | null): BrandSlug | undefined {
  const n = input?.trim().toLowerCase();
  if (!n) return undefined;
  if (n in BRAND_PROFILES) return n as BrandSlug;
  return BRAND_ALIAS_MAP[n];
}

/** 입력(정식/별칭/미지)에 대응하는 프로필. 미지 브랜드는 undefined — 정책 룰이 발화하지 않는다. */
export function getBrandProfile(input?: string | null): BrandProfile | undefined {
  const slug = resolveBrandSlug(input);
  return slug ? BRAND_PROFILES[slug] : undefined;
}
