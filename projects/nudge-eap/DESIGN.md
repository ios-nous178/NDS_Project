...
- **Radius**: 8px (`shape.md` / `radius.8`)
- **카드 패딩(비대칭)**: top 28 / x 16 / bottom 16, 본문 그룹↔버튼 그룹 24px gap, 그룹 내부 8px gap
- **오버레이**: 50% 딤
- **접근성**: 포커스 트랩, ESC 닫기, `role="dialog"` + `aria-modal`
...
설명 + 버튼이 기본 구성. 타이틀과 이미지는 선택적으로 사용합니다.

- `default`: 설명 + Cancel/OK
- `title`: Title + 설명 + Cancel/OK
- `Image`: 64×64 아이콘 + Title + 설명 + Cancel/OK (`Modal.Image` 슬롯)

#### Modal Buttons

최대 2개까지 사용하며 중요도에 따라 디자인이 변경됩니다.

- 1 button: 단일 Primary OK (full-width)
