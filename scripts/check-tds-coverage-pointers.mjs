 ...
 *
 * metadata/tdsComponents.json 은 "프로젝트 4개가 결국 갖춰야 할 목표 컴포넌트" 인벤토리이고,
 ...
 * Project × Component Coverage 보드는 멀쩡히 존재하는 컴포넌트를 "missing(빨강)" 으로 거짓 표시한다.
 *
 * 이 게이트는 그 drift 를 차단한다: 비어있지 않은 모든 `nds` 가 실제 react/html/projectChrome export
 ...
 *
 * 판정 로직(projectChrome 특례 포함)은 coverage-logic.mjs 를 그대로 재사용해 보드와 일치시킨다.
 */
import { readFileSync } from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { buildManifest } from "./coverage-manifest.mjs";
import { PROJECTS } from "./coverage-logic.mjs";
...
function resolves(c) {
  const nds = c.nds;
  // projectChrome 목표는 packages/react/src/{project}/{nds}.tsx 존재로 판정.
  const reactCovered = c.projectChrome ? chromeNames.has(nds) : manifest.reactExports.has(nds);
...
if (dead.length > 0) {
  console.error(
    `[check-tds-coverage-pointers] metadata/tdsComponents.json 의 nds 포인터 ${dead.length}개가 죽었습니다 ` +
      `(react/html/projectChrome 어디에도 없음 → 커버리지 보드가 거짓 missing 표시):`,
  );
  for (const c of dead) {
    console.error(`  ✗ ${c.tds} → nds:"${c.nds}"${c.projectChrome ? " (projectChrome)" : ""}`);
  }
  console.error(
