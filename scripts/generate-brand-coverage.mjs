 ...
 * 브랜드 커버리지 데이터 출처 생성기.
 *
 * 보드/표 렌더링은 공유 컴포넌트(@nudge-design/catalog 의 BrandCoverageTable)가 담당하고,
 * docs 페이지(docs/components/brand-coverage.mdx)는 그 컴포넌트를 직접 렌더하는 hand-written mdx.
...
if (checkMode) {
  ...
  if (current !== manifestBody) {
    console.error(
      "[generate-brand-coverage] metadata/coverage-manifest.json 이 stale 합니다. Run `pnpm generate:brand-coverage`.",
    );
    process.exit(1);
  }
  console.log(
    `[generate-brand-coverage] up to date (${path.relative(rootDir, manifestJsonPath)})`,
  );
} else {
